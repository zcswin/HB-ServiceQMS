...
public interface DailySummaryMapper extends BaseMapper<DailySummary> {
    ...
    @Select("<script>" +
            ...
            "</script>")
    List<DailySummary> listDailySummaryPage(
            @Param("ew") QueryWrapper<DailySummary> ew,
            @Param("pageOffset") int pageOffset,
            @Param("pageSize") int pageSize
    );

    @Select("SELECT COUNT(*) FROM daily_summary da  ${ew.customSqlSegment}")
    int Count(@Param(Constants.WRAPPER) QueryWrapper<DailySummary> qw);
