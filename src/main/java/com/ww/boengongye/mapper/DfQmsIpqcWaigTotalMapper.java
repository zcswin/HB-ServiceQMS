 ...
 * @since 2022-09-16
 */
public interface DfQmsIpqcWaigTotalMapper extends BaseMapper<DfQmsIpqcWaigTotal> {
    ...
    @Select("select little_num.flaw_name name, little_num.date date, avg(if(ng_num is null or all_num is null, 0, ng_num / all_num)) * 100 as rate, if(avg(ng_num) is null, 0, avg(ng_num)) ng_num, avg(all_num) all_num from " +
            ...
            "from df_qms_ipqc_waig_total tol " +
            "left join df_qms_ipqc_waig_detail det on tol.id = det.f_parent_id  " +
    ...
    @Select("select t2.* " +
            ...
            "left join df_qms_ipqc_waig_total tol on tol.id = det.f_parent_id     " +
    ...
    /*@Select("select date_format(DATE_SUB(f_time, INTERVAL 7 HOUR), '%m-%d') date, sum(spot_check_count - affect_count) / sum(spot_check_count) rate    " +
            ...
            "group by date " +
            "order by date ")*/  // 去掉破片前
    @Select("select date_format(date, '%m-%d') date ,rate from(select date_format(DATE_SUB(tol.f_time, INTERVAL 7 HOUR), '%y-%m-%d') date, sum(spot_check_count - if(det.f_sort != '破片',affect_count,0)) / sum(spot_check_count) rate    " +
            "from df_qms_ipqc_waig_total tol " +
            "left join df_qms_ipqc_waig_detail det on tol.id = det.f_parent_id   " +
            "${ew.customSqlSegment} " +
            "group by date " +
    ...
    @Select("<script>" +
            ...
            "      DATE_FORMAT(DATE_SUB(tol.f_time, INTERVAL 7 HOUR), '%Y-%m-%d') AS time_period,\n" +
            "      'DETAIL' AS data_type,\n" +
            "      tol.f_time AS f_time,\n" +
            "    </when>\n" +
            "    <when test='period == \"week\"'>\n" +
            "      CONCAT(YEAR(DATE_SUB(tol.f_time, INTERVAL 7 HOUR)), '-WK', \n" +
            "      WEEK(DATE_SUB(tol.f_time, INTERVAL 7 HOUR), 1)) AS time_period,\n" +
            "      'SUMMARY' AS data_type,\n" +
            "      tol.f_time AS f_time,\n" +
            "    </when>\n" +
            "    <when test='period == \"month\"'>\n" +
            "      DATE_FORMAT(DATE_SUB(tol.f_time, INTERVAL 7 HOUR), '%Y-%m') AS time_period,\n" +
            "      'SUMMARY' AS data_type,\n" +
            "      tol.f_time AS f_time,\n" +
            "    </when>\n" +
            "    <when test='period == \"year\"'>\n" +
            "      YEAR(DATE_SUB(tol.f_time, INTERVAL 7 HOUR)) AS time_period,\n" +
            "      'SUMMARY' AS data_type,\n" +
            "      tol.f_time AS f_time,\n" +
            "    </when>\n" +
            "  </choose>\n" +
            "  SUM(spot_check_count) AS spot_check_count,\n" +
            "  SUM(spot_check_count - affect_count) AS okNum,\n" +
            "  SUM(affect_count) AS ngNum,\n" +
            "  CASE \n" +
            "    WHEN SUM(spot_check_count) > 0 THEN \n" +
            "      SUM(spot_check_count - IF(det.f_sort != '破片', affect_count, 0)) / SUM(spot_check_count)\n" +
            "    ELSE 1 \n" +
            "  END AS okRate\n" +
            "FROM df_qms_ipqc_waig_total tol\n" +
            "LEFT JOIN df_qms_ipqc_waig_detail det ON tol.id = det.f_parent_id\n" +
            "WHERE DATE_SUB(tol.f_time, INTERVAL 7 HOUR) BETWEEN #{startTime} AND #{endTime}\n" +
            ...
            "  AND WEEKDAY(DATE_SUB(tol.f_time, INTERVAL 7 HOUR)) != 6\n" +
            "  AND spot_check_count IS NOT NULL\n" +
            "GROUP BY time_period,tol.f_time\n" +
...
//            "left join      " +
//            "( SELECT  " +
//            " f_sort  , f_seq ,count(f_sort) as ng_num from " +
//            " ( " +
//            " SELECT " +
//            " distinct(f_sort ) f_sort, " +
//            " f_seq, " +
//            " f_product_id " +
//            " FROM " +
//            " df_qms_ipqc_waig_detail det " +
...
//            " JOIN df_process dp ON dp.process_name = tol.f_seq  " +
//            "where dp.floor like #{floor} and tol.f_seq like #{process} and tol.f_bigpro like #{project} and tol.f_line like #{lineBody} and tol.status = 'NG' and tol.f_time between #{startTime} and #{endTime} and HOUR(DATE_SUB(tol.f_time, INTERVAL 7 HOUR)) between #{startHour} and #{endHour} and det.f_sort != '破片' " +
//            " ) t1 GROUP BY f_sort, f_seq) ng_num on ng_num.f_sort = item.flaw_name) item_res_ng left join " +
//            "(select f_seq,sum(spot_check_count) all_num     " +
//            "from df_qms_ipqc_waig_total     " +
//            "where f_seq like #{process} and f_bigpro like #{project} and f_line like #{lineBody} and f_time between #{startTime} and #{endTime} and HOUR(DATE_SUB(f_time, INTERVAL 7 HOUR)) between #{startHour} and #{endHour} group by f_seq) all_num on item_res_ng.f_seq = all_num.f_seq " +
//            "group by name, f_seq     " +
...
//            "left join df_qms_ipqc_flaw_color clr on t.name = clr.name  ")
    ...
    @Select(" <script> select t.*, clr.color 'date'         " +
            "                        from        " +
            "                        (select name, (ng_num / all_num) rate from     " +
            "       " +
            "                        (select f_sort name, sum(ng_num) ng_num , sum(all_num) all_num from          " +
            "                 " +
            "                        ( SELECT      " +
            ...
            "</foreach>" +
            "</if>" +
            " and tol.f_bigpro like #{project} and tol.f_line like #{lineBody} and tol.status = 'NG' and tol.f_time between #{startTime} and #{endTime} and HOUR(DATE_SUB(tol.f_time, INTERVAL 7 HOUR)) between #{startHour} and #{endHour} and det.f_sort != '破片' " +
            "   GROUP BY det.f_sort, tol.f_seq)     " +
            "                " +
            "           item_res_ng left join     " +
            "                        (select f_seq,sum(spot_check_count) all_num         " +
            "                        from df_qms_ipqc_waig_total    " +
            "where  f_bigpro like #{project} " +
            "<if test='process != null and process.size() > 0'>" +
            " and f_seq in " +
            "<foreach collection='process' item='item' open='(' close=')' separator=','> " +
            "#{item} " +
            "</foreach>" +
            "</if>" +
            "  and f_line like #{lineBody} and f_time between #{startTime} and #{endTime} and HOUR(DATE_SUB(f_time, INTERVAL 7 HOUR)) between #{startHour} and #{endHour} " +
            "  group by f_seq) all_num on item_res_ng.f_seq = all_num.f_seq     " +
            "                        group by name         " +
            "                        ) t      " +
            "                        group by name     " +
            "                        order by rate desc     " +
            "                        limit 10) t     " +
            "                        left join df_qms_ipqc_flaw_color clr on t.name = clr.name   </script>")
        // 加上早晚班判断 //5楼只查5楼数据
    ...
    @Select("select pro_res_rate.*, @total := @total * ok_rate as rate from " +
            ...
            "(select process_name, sort " +
            "from df_process " +
            "where sort is not null and process_name != 'SSB' and floor = '4F' and project like #{project} ) pro " +
            "left join  " +
            "(select f_seq, ok_rate from " +
...
//            "group by f_seq) all_num on ng_item.f_seq = all_num.f_seq order by rate desc")
    ...
    @Select("SELECT  " +
            ...
            " f_seq   " +
            " ) ng_item  " +
            " LEFT JOIN ( SELECT f_seq, sum( spot_check_count ) all_num FROM df_qms_ipqc_waig_total" +
            " where f_bigpro like #{project} and f_line like #{lineBody} and f_time between #{startTime} and #{endTime} and HOUR(DATE_SUB(f_time, INTERVAL 7 HOUR)) between #{startHour} and #{endHour} " +
            "   " +
...
//            "group by f_mac) all_num on ng_item.f_mac = all_num.f_mac order by rate desc ")
    ...
    @Select("select all_num.f_mac name, all_num.all_num, ng_item.ng_num ng_num, ng_item.ng_num/all_num.all_num rate from " +
            ...
            "    tol.f_mac,   " +
            "    det.f_product_id,det.f_parent_id from  df_qms_ipqc_waig_detail det " +
            "left join df_qms_ipqc_waig_total tol on det.f_parent_id = tol.id " +
            "join df_process dp ON dp.process_name = tol.f_seq " +
            "where dp.floor like #{floor} and det.f_sort = #{ngItem} and tol.f_bigpro like #{project} and tol.f_seq like #{process} and tol.f_line like #{lineBody} and tol.f_time between #{startTime} and #{endTime} and HOUR(DATE_SUB(tol.f_time, INTERVAL 7 HOUR)) between #{startHour} and #{endHour} " +
...
//只查5楼
    ...
    @Select("select f_seq name, sum(spot_check_count - affect_count) / sum(spot_check_count) rate from df_qms_ipqc_waig_total " +
            "${ew.customSqlSegment} " +
            "group by f_seq")
    List<Rate> listAllProcessOkRate2(@Param(Constants.WRAPPER) Wrapper<DfQmsIpqcWaigTotal> wrapper);

    @Select("SELECT SUM(spot_check_count)  as  spot_check_count ,sum(affect_count) as  affect_count FROM df_qms_ipqc_waig_total " +
            "${ew.customSqlSegment} ")
    ...
    @Select("select t1.*, t2.rate from ( " +
            "select * from ( " +
            "select DATE_FORMAT(DATE_SUB(f_time,INTERVAL 7 HOUR),'%m-%d') date from df_qms_ipqc_waig_total tol " +
            "${ew.customSqlSegment} ) t, " +
            "(select 'A' day_or_night union select 'B' day_or_night) day_or_night " +
            "group by date, day_or_night) t1 " +
            "left join  " +
            "(select DATE_FORMAT(DATE_SUB(f_time,INTERVAL 7 HOUR),'%m-%d') date, if(hour(DATE_SUB(f_time,INTERVAL 7 HOUR)) > 11, 'B', 'A') day_or_night, sum(affect_count) / sum(spot_check_count) rate from df_qms_ipqc_waig_total tol " +
            "${ew.customSqlSegment}  " +
    ...
    @Select({"<script> " +
            ...
            "\t\t\t\t\t) t\n" +
            "\t\t\t\t\tJOIN df_qms_ipqc_waig_total dqiwt\n" +
    ...
    @Select("select * from \n" +
            ...
            "\n" +
            "left join \n" +
            "(select f_seq, sum(spot_check_count - affect_count) app_all_ok_num from df_qms_ipqc_waig_total \n" +
            "where f_time between #{startTime} and #{endTime}\n" +
            "group by f_seq) t3 on t.process_name = t3.f_seq\n" +
            "\n" +
            "left join \n" +
            "(select tol.f_seq, item.f_sort, sum(tol.affect_count) app_item_ng_num from df_qms_ipqc_waig_detail item \n" +
            "left join df_qms_ipqc_waig_total tol on item.f_parent_id = tol.id\n" +
    ...
    @Select("select week(DATE_SUB(tol.f_time, INTERVAL 7 HOUR), '%m-%d') date, sum(spot_check_count - if(det.f_sort != '破片',affect_count,0)) / sum(spot_check_count) rate    " +
            ...
            "order by date ")
        // 去掉破片后
    List<Rate> listCNC0OkRateOneMonth(@Param(Constants.WRAPPER) QueryWrapper<DfQmsIpqcWaigTotal> qw);

    @Select("SELECT COUNT(*) FROM DF_QMS_IPQC_WAIG_TOTAL total ${ew.customSqlSegment}")
    int Count(@Param(Constants.WRAPPER) QueryWrapper<DfQmsIpqcWaigTotal> qw);
    ...
    @Select("SELECT (sum(spot_check_count-affect_count)/sum(spot_check_count)*100) as rate FROM `df_qms_ipqc_waig_total` wg join df_machine mac on wg.f_mac=mac.code ${ew.customSqlSegment} ")
    ...
    @Select("with temp_date_machine as (\n" +
            ...
            "\tFROM DF_QMS_IPQC_WAIG_TOTAL total\n" +
            ...
            "GROUP BY F_MAC\n" +
            "HAVING count(*) >=6;")
    List<DfQmsIpqcWaigTotal> weekOfPoorTOP3Warning(@Param(Constants.WRAPPER) QueryWrapper<DfQmsIpqcWaigTotal> ew);
    ...
    @Select("with waig_defect as(\n" +
            ...
            "\tfrom df_qms_ipqc_waig_detail dqiwd \n" +
            "\tinner join df_qms_ipqc_waig_total dqiwt\n" +
...
//            "    from DF_QMS_IPQC_WAIG_TOTAL total\n" +
//            "    left join df_qms_ipqc_waig_detail detail ON total.ID = detail.F_PARENT_ID\n" +
//            "${ew.customSqlSegment} " +
...
//            ")\n" +
//            "ORDER BY date DESC;")
//    List<DfQmsIpqcWaigTotal> listWaigExcelData(@Param("ew") QueryWrapper<DfQmsIpqcWaigTotal> ew);

/*    //优化
    @Select("WITH temp_main as(  \n" +
            "                   select date_format(date_sub(total.F_TIME,INTERVAL 7 HOUR ),'%Y-%m-%d') date,total.f_time,F_BIGPRO,f_seq,F_FAC,F_COLOR,f_test_category,f_stage,f_test_type,f_mac,f_Type,f_line,sum(spot_check_count) spot_check_count,  \n" +
            "                      sum(spot_check_count - affect_count) as okNum,sum(spot_check_count - affect_count)/sum(spot_check_count) as okRate,  \n" +
            "                      sum(affect_count) as ngNum,F_TEST_MAN   \n" +
            "                   from DF_QMS_IPQC_WAIG_TOTAL total  \n" +
            "                  inner join df_process dp on dp.process_name = total.f_seq  \n" +
            "                  ${ew.customSqlSegment}  \n" +
            "                   GROUP BY date,total.f_time,total.F_BIGPRO,total.f_seq,total.F_FAC,total.F_COLOR,f_test_category,f_stage,f_test_type,f_mac,f_line,f_Type,F_TEST_MAN  \n" +
            "                   having spot_check_count is not null  \n" +
            "               )  \n" +
            "               , temp_slave AS (  \n" +
            "                   select date_format(date_sub(total.F_TIME,INTERVAL 7 HOUR ),'%Y-%m-%d') date,total.f_time,F_BIGPRO,f_seq,F_FAC,F_COLOR,f_test_category,F_TEST_MAN,F_SORT,f_stage,f_test_type,f_mac,f_Type\n" +
            "\t\t\t\t\t\t\t\t\t ,f_line,count(0) num  \n" +
            "                   from DF_QMS_IPQC_WAIG_TOTAL total  \n" +
            "                  inner join df_process dp on dp.process_name = total.f_seq  \n" +
            "                   left join df_qms_ipqc_waig_detail detail ON total.ID = detail.F_PARENT_ID  \n" +
            "                  ${ew.customSqlSegment}  \n" +
            "                   GROUP BY date,total.f_time,total.F_BIGPRO,total.f_seq,total.F_FAC,total.F_COLOR,f_test_category,f_stage,f_test_type,f_mac,f_Type,f_line,\n" +
            "\t\t\t\t\t\t\t\t\t F_TEST_MAN,F_SORT  \n" +
            "                   having F_SORT is not null \n" +
            "               )  \n" +
            "               select main.date,main.f_time,main.F_BIGPRO,main.f_seq,main.F_FAC,main.F_COLOR,main.f_test_category,spot_check_count,okNum,okRate,ngNum,main.F_TEST_MAN,main.f_stage,main.f_test_type,main.f_mac,main.f_Type,main.f_line,F_SORT,num from(  \n" +
            "                   temp_main main  \n" +
            "                   JOIN temp_slave slave  \n" +
            "                   ON main.date = slave.date  \n" +
            "                   AND main.F_BIGPRO = slave.F_BIGPRO  \n" +
...
//    @Select("WITH temp_main AS (  \n" +
//            "   SELECT total.type, date_format(date_sub(total.F_TIME, INTERVAL 7 HOUR), '%Y-%m-%d') AS date, total.f_time, F_BIGPRO, f_seq, F_FAC, F_COLOR, f_test_category, f_stage, f_test_type, f_mac, f_Type, f_line, \n" +
//            "          SUM(spot_check_count) AS spot_check_count, \n" +
//            "          SUM(spot_check_count - affect_count) AS okNum, \n" +
//            "          SUM(spot_check_count - affect_count) / SUM(spot_check_count) AS okRate, \n" +
//            "          SUM(affect_count) AS ngNum, F_TEST_MAN, \n" +
//            "          CASE \n" +
//            "            WHEN HOUR(date_sub(total.F_TIME, INTERVAL 7 HOUR)) >= 7 AND HOUR(date_sub(total.F_TIME, INTERVAL 7 HOUR)) < 19 THEN '白班' \n" +
//            "            ELSE '晚班' \n" +
//            "          END AS shift \n" +
//            "   FROM (select * from DF_QMS_IPQC_WAIG_TOTAL total  ${ew.customSqlSegment} LIMIT #{pageOffset}, #{pageSize}) total \n" +
//            "   INNER JOIN df_process dp ON dp.process_name = total.f_seq \n" +
//            "and dp.floor = #{floor}"+
//            "   ${ew.customSqlSegment} \n" +
//            "   GROUP BY total.type, date, total.f_time, total.F_BIGPRO, total.f_seq, total.F_FAC, total.F_COLOR, f_test_category, f_stage, f_test_type, f_mac, f_line, f_Type, F_TEST_MAN, shift \n" +
...
//            "), \n" +
//            "temp_slave AS ( \n" +
//            "   SELECT total.type, date_format(date_sub(total.F_TIME, INTERVAL 7 HOUR), '%Y-%m-%d') AS date, total.f_time, F_BIGPRO, f_seq, F_FAC, F_COLOR, f_test_category, F_TEST_MAN, F_SORT, f_stage, f_test_type, f_mac, f_Type, \n" +
...
//            "   and dp.floor = #{floor}"+
//            "   ${ew.customSqlSegment} \n" +
//            "   GROUP BY total.type, date, total.f_time, total.F_BIGPRO, total.f_seq, total.F_FAC, total.F_COLOR, f_test_category, f_stage, f_test_type, f_mac, f_Type, f_line, F_TEST_MAN, F_SORT, shift \n" +
//            "   HAVING F_SORT IS NOT NULL \n" +
//            ") \n" +
//            "SELECT main.type, main.date, main.f_time, main.F_BIGPRO, main.f_seq, main.F_FAC, main.F_COLOR, main.f_test_category, spot_check_count, okNum, okRate, ngNum, main.F_TEST_MAN, main.f_stage, main.f_test_type, main.f_mac, main.f_Type, main.f_line, F_SORT, num, main.shift \n" +
//            "FROM temp_main main \n" +
//            "JOIN temp_slave slave \n" +
//            "  ON main.date = slave.date \n" +
//            "  AND main.F_BIGPRO = slave.F_BIGPRO \n" +
...
//    @Select("   SELECT total.type, date_format(date_sub(total.F_TIME, INTERVAL 7 HOUR), '%Y-%m-%d') AS date, total.f_time,dp.sort AS sort, F_BIGPRO, f_seq, F_FAC, F_COLOR, f_test_category, F_TEST_MAN, F_SORT, f_stage, f_test_type, f_mac, f_Type, \n" +
//            "          f_line, COUNT(0) AS num, \n" +
//            "SUM(DISTINCT spot_check_count) AS spot_check_count," +
//            "SUM(DISTINCT spot_check_count - affect_count) AS okNum," +
//            "SUM(DISTINCT spot_check_count - affect_count) / SUM(DISTINCT spot_check_count) AS okRate," +
...
//            "    FROM df_process_project_config where project like #{project}  \n" +
//            ") dp ON dp.process_name = total.f_seq and dp.project = total.F_BIGPRO \n" +
//            "LEFT JOIN df_qms_ipqc_waig_detail detail ON total.ID = detail.F_PARENT_ID "+
...
//                                                   @Param("project") String project);
    ...
    @Select("SELECT " +
            "total.F_BIGPRO, " +
        "total.f_seq, " +
        "DATE_FORMAT(total.F_TIME, '%Y-%m-%d') AS date, " +
        //"SUM(total.spot_check_count) AS spot_check_count, " +
        //"SUM(total.spot_check_count - total.affect_count) AS okNum, " +
        //"SUM(total.spot_check_count - total.affect_count) / SUM(total.spot_check_count) AS okRate, " +
       // "SUM(total.affect_count) AS ngNum, " +
      //  "COUNT(DISTINCT dm.id) AS machine_num, " +
        "CASE " +
                "    WHEN fr.fai_total !=0 THEN (fr.fai_total - fai_ng) / fr.fai_total " +
                "    ELSE 0 " +
                "END AS fai_startup_rate, " +
        "CASE " +
                "    WHEN fr.cpk_total != 0 THEN (fr.cpk_total - fr.cpk_ng) / fr.cpk_total " +
                "    ELSE 0 " +
                "END AS cpk_pass_rate " +
        "FROM df_qms_ipqc_waig_total total " +
                "LEFT JOIN df_process_project_config dp ON dp.process_name = total.f_seq " +
                "LEFT JOIN df_machine dm ON dm.process_code = total.f_seq " +
                "LEFT JOIN ( " +
                "    SELECT " +
                "        DATE_FORMAT(total.F_TIME, '%Y-%m-%d') AS date, " +
                "        total.F_BIGPRO, " +
                ...
                "        COUNT(DISTINCT dm.id) AS machine_num " +
                "    FROM df_qms_ipqc_waig_total total " +
                "    INNER JOIN df_machine dm ON dm.process_code = total.f_seq " +
                "    WHERE total.f_test_category = 'FAI' " +
                "    GROUP BY date, total.F_BIGPRO, total.f_seq " +
                ") mc ON mc.date = DATE_FORMAT(total.F_TIME, '%Y-%m-%d') " +
                "AND mc.F_BIGPRO = total.F_BIGPRO " +
                "AND mc.f_seq = total.f_seq " +
                "LEFT JOIN ( " +
                "    SELECT " +
                "        DATE_FORMAT(total.F_TIME, '%Y-%m-%d') AS date, " +
                "        total.F_BIGPRO, " +
                "        total.f_seq, " +
                "       SUM(CASE WHEN total.f_test_category IN ('FAI', '开机首检') THEN total.spot_check_count ELSE 0 END) AS fai_total," +
                "        SUM(CASE WHEN total.f_test_category IN ('FAI', '开机首检') THEN total.affect_count ELSE 0 END) AS fai_ng, " +
                "        SUM(CASE WHEN total.f_test_category != 'FAI' THEN total.spot_check_count ELSE 0 END) AS cpk_total, " +
                "        SUM(CASE WHEN total.f_test_category != 'FAI' THEN total.affect_count ELSE 0 END) AS cpk_ng " +
                "    FROM df_qms_ipqc_waig_total total " +
                "    WHERE total.f_test_category IN ('FAI', 'CPK','开机首检') " +
                "    GROUP BY date, total.F_BIGPRO, total.f_seq " +
                ") fr ON fr.date = DATE_FORMAT(total.F_TIME, '%Y-%m-%d') " +
                "AND fr.F_BIGPRO = total.F_BIGPRO " +
                "AND fr.f_seq = total.f_seq " +
                "WHERE " +
                "    DATE_FORMAT(total.F_TIME, '%Y-%m-%d') BETWEEN #{startDate} AND #{endDate} " +
                "    AND HOUR(total.F_TIME) BETWEEN #{startHour} AND #{endHour} " +
//                "    AND total.f_seq = #{processName} " +
//                "    AND total.F_BIGPRO = #{projectName} " +
                "GROUP BY date, total.F_BIGPRO, total.f_seq, mc.machine_num, fr.cpk_total, fr.cpk_ng,fr.fai_total,fr.fai_ng " +
                "ORDER BY date DESC")
...
//            "\tsum(if(affect_count = 'NG',1,0)) as ng\n" +
            ...
            "\tORDER BY date DESC\n" +
            ") as t")
    List<Map<String, Object>> exportInspectionTableForProcess(@Param("ew") QueryWrapper<DfQmsIpqcWaigDetail> waigWrapper);
    ...
    @Select("select *,ROUND( 1 - (ng / total), 2)*100 as qa\n" +
            ...
            "\tORDER BY date DESC\n" +
            ") as t")
    List<Map<String, Object>> exportInspectionTableForProcessBymachineId(@Param("ew") QueryWrapper<DfQmsIpqcWaigDetail> waigWrapper);

    @Select("select *,ROUND( (ng / total), 2)*100 as qa\n" +
            "from (\n" +
            "\tselect \n" +
            "\tdate_format(DATE_SUB(df_qms_ipqc_waig_detail.f_time, INTERVAL 7 HOUR), '%Y-%m-%d') date,\n" +
            "\tf_bigpro as project,\n" +
            "\t'外观' as type,\n" +
            "\tf_seq as process,\n" +
            "\tdf_qms_ipqc_waig_detail.f_sort as item,\n" +
            "\tsum(spot_check_count) as total,\n" +
            "\tsum(if(f_result = 'fail',1,0)) as ng\n" +
            "\tfrom df_qms_ipqc_waig_total\n" +
            "\tLEFT JOIN df_qms_ipqc_waig_detail ON df_qms_ipqc_waig_total.id = df_qms_ipqc_waig_detail.f_parent_id\n" +
            "${ew.customSqlSegment} " +
            "\tORDER BY date DESC\n" +
            ") as t")
    List<Map<String, Object>> exportNgClassificationScale(@Param("ew") QueryWrapper<DfQmsIpqcWaigDetail> waigWrapper);
    ...
    @Select("select * from (SELECT\n" +
            ...
            "\t\n" +
            "FROM\n" +
            "\tdf_qms_ipqc_waig_detail de\n" +
            "\tLEFT JOIN df_qms_ipqc_waig_total tol ON de.f_parent_id = tol.id LEFT\n" +
