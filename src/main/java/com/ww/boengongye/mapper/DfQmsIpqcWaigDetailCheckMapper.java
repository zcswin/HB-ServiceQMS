 ...
 * @since 2024-12-05
 */
public interface DfQmsIpqcWaigDetailCheckMapper extends BaseMapper<DfQmsIpqcWaigDetailCheck> {
    @Select("select d.*,c.color,p.f_barcode,p.f_bigpro,p.f_mac" +
            " from df_qms_ipqc_waig_detail_check d left join df_qms_ipqc_flaw_color c on d.f_sort=c.name  " +
            " left join df_qms_ipqc_waig_total_check p on d.f_parent_id=p.id  " +
    ...
    @Select("select k.f_sm_area,count(k.id) id from (select d.f_sm_area, d.id from df_qms_ipqc_waig_detail_check d  join df_qms_ipqc_waig_total_check p on d.f_parent_id=p.id " +
            "join df_process dp ON dp.process_name = p.f_seq " +
    ...
    //    @Selqect("select SUM(spot_check_count) as id from df_qms_ipqc_waig_total_check join ${ew.customSqlSegment}  ")
    //只查5楼工序
    @Select("select SUM(spot_check_count) as id from df_qms_ipqc_waig_total_check tol join df_process dp ON dp.process_name = tol.f_seq ${ew.customSqlSegment}  ")
    ...
    @Select({"<script> " +
            ...
            "),\n" +
            "\n" +
            "temp_df_qms_ipqc_waig_total_check AS(\n" +
            "\tselect sum(spot_check_count) as waig_total from df_qms_ipqc_waig_total_check t2 where 1 =1 \n" +
            ...
            "\t,(select waig_total from temp_df_qms_ipqc_waig_total_check) as waig_total\n" +
            "\tFROM df_qms_ipqc_waig_detail_check t\n" +
            "\tJOIN df_qms_ipqc_waig_total_check t2 on t.f_parent_id = t2.id\n" +
            "\twhere 1 = 1\n" +
    ...
    @Select("select \n" +
            ...
            "from df_qms_ipqc_waig_total_check dqiwt \n" +
    ...
    @Select("select f_seq str1,f_bigpro str2,f_big_area str3,f_sort str4,rate dou1\n" +
            "FROM(\n" +
            "\tselect F_SEQ,F_BIGPRO,F_BIG_AREA,F_SORT,sum(if(DF_QMS_IPQC_WAIG_DETAIL_CHECK.F_RESULT = 'ok',1,0)) ok,count(*) total,\n" +
            "       sum(if(DF_QMS_IPQC_WAIG_DETAIL_CHECK.F_RESULT = 'ok',1,0)) / count(*) rate\n" +
            "\tFROM DF_QMS_IPQC_WAIG_TOTAL_CHECK\n" +
            "\tJOIN DF_QMS_IPQC_WAIG_DETAIL_CHECK ON DF_QMS_IPQC_WAIG_TOTAL_CHECK.ID = DF_QMS_IPQC_WAIG_DETAIL_CHECK.F_PARENT_ID\n" +
            "\twhere DF_QMS_IPQC_WAIG_DETAIL_CHECK.F_TIME BETWEEN date_add(now(),INTERVAL -1 HOUR) AND now()\n" +
            "\tgroup by DF_QMS_IPQC_WAIG_TOTAL_CHECK.F_SEQ,DF_QMS_IPQC_WAIG_TOTAL_CHECK.F_BIGPRO,DF_QMS_IPQC_WAIG_DETAIL_CHECK.F_BIG_AREA,DF_QMS_IPQC_WAIG_DETAIL_CHECK.F_SORT\n" +
            ") t\n" +
    ...
    @Select("SELECT DISTINCT\n" +
            ...
            "count(*) AS count \n" +
            "FROM\n" +
            "df_qms_ipqc_waig_total_check total\n" +
            "left join df_qms_ipqc_waig_detail_check detail ON detail.f_parent_id = total.id \n" +
