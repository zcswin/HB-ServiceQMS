...
public class CheckSizeProcessYield {
    ...
    public void checkProcessYield() throws ParseException {
        if(env.getProperty("CcYieldWarn","N").equals("Y")){
            ...
            for (DfProcessProjectConfig p : processList) {
                ...
                qw3.eq("process", p.getProcessName());
//            qw3.between("create_time", TimeUtil.getYesterday(), TimeUtil.getYesterday() + " 23:59:59");
                qw3.between("create_time", TimeUtil.getBeforeDay(7), TimeUtil.getNowTimeByNormal());
                Rate data = dfSizeDetailService.getSizeRealRate(qw3);
                if (null != data && null != data.getRate() &&data.getRate()!=0) {


                    double processYield=data.getRate() * 100.0;
