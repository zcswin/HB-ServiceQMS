...
public class CheckAppearanceLineYield {
...
//     @Scheduled(cron = "0 15 10 ? * MON")
    public void checkProcessYield() throws ParseException {
        if(env.getProperty("WgYieldWarn","N").equals("Y")){
            List<LineBody> lineList = lineBodyService.list();
            ...
            for(LineBody line:lineList){
                QueryWrapper<DfQmsIpqcWaigTotal> qw3 = new QueryWrapper<>();
                qw3.eq("mac.linebody_id",line.getId());
//            qw3.between("wg.f_time", TimeUtil.getBeforeDay(7),TimeUtil.getYesterday() + " 23:59:59");
                qw3.between("wg.f_time", TimeUtil.getBeforeDay(7),TimeUtil.getNowTimeByNormal() );
                Rate rate=dfQmsIpqcWaigTotalService.getLineYield(qw3);
                if(null!=rate&&rate.getRate()>0.0&&null!=yield&&null!=yield.getAlarmValue()&&null!=yield.getPrewarningValue()){
                    double processYield = rate.getRate() ;
                    if (processYield>0 &&  processYield< Double.valueOf(yield.getAlarmValue()+"") ||  processYield < Double.valueOf(yield.getPrewarningValue()+"")) {
                        ...
                        sqw.eq("type", "appearanceLineYield");
...
//                        aud.setParentId(dd.getId());
                            ...
                            if (processYield < yield.getAlarmValue()) {
                                ...
                                aud.setQuestionName(line.getName()+"线体良率:"+decimalFormat.format(processYield)+"%低于" + yield.getAlarmValue() + "%");
                                aud.setScenePractical(line.getName()+"线体良率" + ":"+decimalFormat.format(processYield)+"%低于" + yield.getAlarmValue() + "% "+ "," +TimeUtil.getBeforeDayMMdd(7)+"至"+ TimeUtil.getYesterdayNoYear() +",请及时处理");
                                fd.setName("工序良率报警_" +line.getName()+ "线体良率"  + ":"+decimalFormat.format(processYield)+"%低于" + yield.getAlarmValue() + "%"+ ","  +TimeUtil.getBeforeDayMMdd(7)+"至"+ TimeUtil.getYesterdayNoYear());
                            } else {
                                aud.setQuestionType("良率预警");
                                aud.setControlStandard("良率预警:低于" + yield.getPrewarningValue() + "%");
                                aud.setQuestionName(line.getName()+"线体良率" + ":"+decimalFormat.format(processYield)+"%低于" + yield.getPrewarningValue() + "%"+ "_" + TimeUtil.getYesterdayNoYear() );
                                aud.setScenePractical(line.getName()+"线体良率"  + ":"+decimalFormat.format(processYield)+"%低于" + yield.getPrewarningValue() + "%"+ "," +TimeUtil.getBeforeDayMMdd(7)+"至"+ TimeUtil.getYesterdayNoYear()+",请及时处理");
                                fd.setName("工序良率预警_" + line.getName()+"线体良率"  + ":"+decimalFormat.format(processYield)+"%低于" + yield.getPrewarningValue() + "%"+ ","  +TimeUtil.getBeforeDayMMdd(7)+"至"+ TimeUtil.getYesterdayNoYear());
                            }

                            aud.setDepartment(line.getName());
                            aud.setAffectMac("0");
//                        aud.setAffectNum(1.0);

                            aud.setImpactType("线体良率");
                            aud.setIsFaca("0");
                            //问题名称和现场实际调换
