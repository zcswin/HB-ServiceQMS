...
@Component
public class IpqcWaigTotalTask {
    private static final Logger logger = LoggerFactory.getLogger(IpqcWaigTotalTask.class);
    ...
    DailySummaryService dailySummaryService = BeanUtils.getBean(DailySummaryServiceImpl.class);
    ...
    public void generateWeeklyStats(LocalDate weekStart) {
        ...
        // 第一部分: 查询原始数据
        String type1 = "f1";
        List<DailySummary> dailyList = getDailySummaryData(weekStart, weekEnd, type1);
        ...
        // 第二部分: 查询原始数据 f2
        String type = "f2";
        List<DailySummary> dailyData = getDailySummaryData(weekStart, weekEnd, type);

        // 聚合工序数据和缺陷数据
        Map<DailySummary, DailySummary> processMap = aggregateProcessData(dailyData);
        Map<DailySummary, Map<String, Integer>> defectMap = aggregateDefectData(dailyData);
        ...
        // 第三部分: 查询原始数据 f3
        String type3 = "f3";
        List<DailySummary> dailyProcessDataAll = getDailySummaryData(weekStart, weekEnd, type3);

        // 聚合工序数据和缺陷数据
        Map<DailySummary, DailySummary> processMapAll = aggregateProcessData(dailyProcessDataAll);
        Map<DailySummary, Map<String, Integer>> defectMapAll = aggregateDefectData(dailyProcessDataAll);
    ...
    // 获取指定数据类型的 DailySummary 数据
    private List<DailySummary> getDailySummaryData(LocalDate start, LocalDate end, String dataType) {
        return dailySummaryMapper.selectList(
                new QueryWrapper<DailySummary>()
                        .select("*")
    ...
    // 按业务维度分组，不按班次分组
    private Map<String, List<DailySummary>> groupByBizKeyAndShift(List<DailySummary> dailyList) {
    ...
    // 聚合工序数据
    private Map<DailySummary, DailySummary> aggregateProcessData(List<DailySummary> dailyData) {
        Map<DailySummary, DailySummary> processMap = new HashMap<>();
        for (DailySummary daily : dailyData) {
            DailySummary key = buildDimensionKey(daily);

            processMap.compute(key, (k, v) -> {
                if (v == null) {
                    v = createDefaultDailySummary(k);
                }
    ...
    // 聚合缺陷数据
    private Map<DailySummary, Map<String, Integer>> aggregateDefectData(List<DailySummary> dailyData) {
        Map<DailySummary, Map<String, Integer>> defectMap = new HashMap<>();
        for (DailySummary daily : dailyData) {
            DailySummary key = buildDimensionKey(daily);
    ...
    // 创建默认的 DailySummary 实例
    private DailySummary createDefaultDailySummary(DailySummary key) {
        DailySummary v = new DailySummary();
    ...
    // 累加工序数据
    private void accumulateProcessData(DailySummary v, DailySummary daily) {
    ...
    private void processShiftStats(LocalDate start, LocalDate end, String periodLabel, String statisticType, String dataType, List<String> defect, String dateType) {
        ...
        for (int i = 0; i < yearMonths.size(); i += BATCH_SIZE) {
            ...
            // 查询当前批次的数据
            List<DailySummary> dailyList = dailySummaryMapper.selectList(
                    new QueryWrapper<DailySummary>()
    ...
    private void processShiftStatsMonth(LocalDate start, LocalDate end, String periodLabel, String statisticType, String dataType, List<String> defect, String dateType) {
        ...
        int batchSize = 1000;
        int pageNum = 1;
        List<DailySummary> dailyList;

        try {
            // 分页查询数据
            do {
                // 通过数据库分页查询返回数据
                Page<DailySummary> page = new Page<>(pageNum, batchSize);
                QueryWrapper<DailySummary> queryWrapper = new QueryWrapper<DailySummary>()
                        .between("date", start, end)
                        .eq("data_type", dataType)
                        .eq("statistic_type", statisticType)
                        .in("shift", "白班", "晚班");
                ...
                // 查询分页数据
                dailyList = dailySummaryMapper.selectPage(page, queryWrapper).getRecords();
    ...
    private void storeBatchData(List<DailySummary> dailyList, List<String> defect, String dataType, String periodLabel,LocalDate start,LocalDate end) {
        try {
            // 预加载缺陷数据
            loadDefectsForDailyData(dailyList);
    ...
    private void processProcessStatsMonth(LocalDate start, LocalDate end, String periodLabel,String statisticType,String dataType,String dateType) {
        // 查询数据
        List<DailySummary> dailyData = dailySummaryMapper.selectList(
                new QueryWrapper<DailySummary>()
                        .between("date", start, end)
                        .eq("data_type", dataType)
                        .eq("statistic_type", statisticType)
    ...
    // 核心工具方法
    private DailySummary buildDimensionKey(DailySummary daily) {
    ...
    private void aggregateProcessData(Map<DailySummary, DailySummary> processMap,
                                      Map<DailySummary, Map<String, Integer>> defectMap,
                                      DailySummary daily, DailySummary key) {
        processMap.compute(key, (k, v) -> {
            if (v == null) {
    ...
    private void aggregateFactoryData(Map<DailySummary, DailySummary> processMap,
                                      Map<DailySummary, Map<String, Integer>> defectMap,
                                      DailySummary daily, DailySummary key) {
        processMap.compute(key, (k, v) -> {
            if (v == null) {
    ...
    private DailySummary createBaseSummary(DailySummary k) {
    ...
    private DailySummary updateSummaryValues(DailySummary summary, DailySummary daily) {
    ...
    private void saveAggregatedData(Map<DailySummary, DailySummary> processMap,
                                    Map<DailySummary, Map<String, Integer>> defectMap,
        ...
        processMap.forEach((dimension, monthly) -> {
            try {
                ...
                // 保存主记录
                dailySummaryService.save(monthly);
    ...
    private String getBizKey(DailySummary d) {
        return Stream.of(
                        d.getFStage(),
                        d.getFBigpro(),
                        d.getFColor(),
    ...
    // 加载缺陷数据
    private void loadDefectsForDailyData(List<DailySummary> dailyList) {
    ...
    private void processDefects(Map<String, List<DailySummary>> shiftMap, Integer mainId) {
        // 合并所有班次缺陷
    ...
    private void saveShiftDefects(Integer mainId, String shiftType, Map<String, Integer> defects) {
        // 获取总检数（用于计算缺陷率）
        Integer totalChecked = dailySummaryMapper.selectById(mainId).getSpotCheckCount();
    ...
    private DailySummary saveWeeklyMain(Map<String, List<DailySummary>> shiftMap,
                                        String periodLabel,
                                        LocalDate start, LocalDate end,String type) {
        // 获取样本数据
        DailySummary sample = shiftMap.values().iterator().next().get(0);

        // 创建周统计主记录
        DailySummary weekly = new DailySummary();
        ...
        // 保存主记录
        dailySummaryMapper.insert(weekly);
        return weekly;
    }

    private void calculateWeeklyValues(Map<String, List<DailySummary>> shiftMap, DailySummary weekly) {
        // 合并所有班次数据
        List<DailySummary> allShifts = shiftMap.values().stream()
                .flatMap(List::stream)
                .collect(Collectors.toList());

        // 累加数值字段
        weekly.setSpotCheckCount(allShifts.stream().mapToInt(DailySummary::getSpotCheckCount).sum());
        weekly.setOkNum(allShifts.stream().mapToInt(DailySummary::getOkNum).sum());
        weekly.setNgNum(allShifts.stream().mapToInt(DailySummary::getNgNum).sum());
    ...
    public void calculateDailyStats(LocalDate date) {
        ...
        QueryWrapper<DfQmsIpqcWaigTotal> ew = new QueryWrapper<>(); // 创建查询包装器
        ...
        List<List<DfQmsIpqcWaigTotal>> datalist = getOptimizedDataWithCustomExecutor(ew, totalCount, floor);
        List<DfQmsIpqcWaigTotal> list = datalist.get(0); // 获取外观数据
    ...
    // 公共方法：处理单个班次的数据
    private void storeShiftData(LocalDate date, String shift, String processName, List<String> defect, String type, List<DfQmsIpqcWaigTotal> list) {
    ...
    public List<List<DfQmsIpqcWaigTotal>> getOptimizedDataWithCustomExecutor(QueryWrapper<DfQmsIpqcWaigTotal> ew, int totalCount, String floor) {
        // 创建一个固定大小的线程池
    ...
    public List<DfQmsIpqcWaigTotal> getPagedData(QueryWrapper<DfQmsIpqcWaigTotal> ew, int totalCount, String floor) throws InterruptedException, ExecutionException {
        ...
        for (int page = 1; page <= totalPages; page++) {
            ...
            CompletableFuture<List<DfQmsIpqcWaigTotal>> future = CompletableFuture.supplyAsync(() -> {
                try {
                    return dfQmsIpqcWaigTotalService.listWaigExcelDataPage(ew, pageOffset, pageSize, floor, "%%");
                } catch (Exception e) {
                    // 捕获每个查询任务中的异常，并打印日志
    ...
    private List<Map<String, Object>> aggregateDayDataf1(List<DfQmsIpqcWaigTotal> list, String day, String processName, String shift, List<String> defectList, String type) {
        List<Map<String, Object>> result = new ArrayList<>();
        // 过滤出符合条件的记录
        List<DfQmsIpqcWaigTotal> filtered = list.stream()
        ...
        // 按多个字段进行分组
        Map<String, List<DfQmsIpqcWaigTotal>> collect = filtered.stream().collect(Collectors.groupingBy(total -> {
...
//        }
        ...
        collect.forEach((key, iems) -> {
            // 按班次分组
            Map<String, List<DfQmsIpqcWaigTotal>> shiftGroups = iems.stream()
                    .collect(Collectors.groupingBy(DfQmsIpqcWaigTotal::getShift));
    ...
    private List<Map<String, Object>> aggregateDayDataf3(List<DfQmsIpqcWaigTotal> list, String day, String processName, String shift, List<String> defectList, String type) {
        List<Map<String, Object>> result = new ArrayList<>();
        // 过滤出符合条件的记录
        List<DfQmsIpqcWaigTotal> filtered = list.stream()
                .filter(t -> (day == null || day.equals(t.getDate())))
...
//        }
    ...
    private List<Map<String, Object>> aggregateDataWeek(List<DailySummary> list, List<String> defectList, String type) {
        List<Map<String, Object>> result = new ArrayList<>();


        // 按多个字段进行分组
        Map<String, List<DailySummary>> collect = list.stream().collect(Collectors.groupingBy(total -> {
            return total.getFStage() + "," + total.getFBigpro() + "," + total.getFColor() + "," + total.getFFac() + ","
                    + total.getFType() + "," + total.getDate() + "," + total.getShift() + "," + total.getFSeq()
                    + "," + total.getFTestCategory() + "," + total.getFLine() + "," + total.getFMac() + "," + total.getFTestType() + "," + total.getFTestMan() + "," + total.getFSort();
        }));
        ...
        collect.forEach((key, iems) -> {
            // 按班次分组
            Map<String, List<DailySummary>> shiftGroups = iems.stream()
                    .collect(Collectors.groupingBy(DailySummary::getShift));
            String[] keyParts = key.split(","); // 提取键的各部分
    ...
    private void processTotal(List<Map<String, Object>> result, String[] keyParts,
                              List<DfQmsIpqcWaigTotal> dayShift,
                              List<DfQmsIpqcWaigTotal> nightShift,
                              List<String> defects) {
        ...
        // 合并数据
        int totalCheck = dayShift.stream().mapToInt(DfQmsIpqcWaigTotal::getSpotCheckCount).sum() +
                nightShift.stream().mapToInt(DfQmsIpqcWaigTotal::getSpotCheckCount).sum();
    ...
    private void processTotalWeek(List<Map<String, Object>> result, String[] keyParts,
                                  List<DailySummary> dayShift,
                                  List<DailySummary> nightShift,
                                  List<String> defects) {
        Map<String, Object> totalMap = new LinkedHashMap<>();
        // 基础信息
        totalMap.put("fStage", keyParts[0]);  // 阶段
        ...
        // 合并数据
        int totalCheck = dayShift.stream().mapToInt(DailySummary::getSpotCheckCount).sum() +
                nightShift.stream().mapToInt(DailySummary::getSpotCheckCount).sum();
    ...
    private void processTotal3(List<Map<String, Object>> result, String[] keyParts,
                               List<DfQmsIpqcWaigTotal> allShifts, List<String> defects) {
        Map<String, Object> totalMap = new LinkedHashMap<>();
        // 基础信息
        totalMap.put("fStage", keyParts[0]);  // 阶段
        totalMap.put("fBigpro", keyParts[1]);  // 项目
        ...
        // 合并数据，直接对所有班次的数据进行计算
        int totalCheck = allShifts.stream().mapToInt(DfQmsIpqcWaigTotal::getSpotCheckCount).sum();
    ...
    private void processShift(List<Map<String, Object>> result, String[] keyParts,
                              List<DfQmsIpqcWaigTotal> items, List<String> defects, String shift) {
        ...
        // 统计数值
        int totalCheck = items.stream().mapToInt(DfQmsIpqcWaigTotal::getSpotCheckCount).sum();
    ...
    private void processShiftWeek(List<Map<String, Object>> result, String[] keyParts,
                                  List<DailySummary> items, List<String> defects, String shift) {
        // if (items.isEmpty()) return;
        ...
        // 统计数值
        int totalCheck = items.stream().mapToInt(DailySummary::getSpotCheckCount).sum();
    ...
    private void storeAllDailyData(LocalDate date, String shift, String processName,
        ...
        for (Map<String, Object> dayData : dailyDataList) {
            ...
            dailySummaryService.save(summary);
