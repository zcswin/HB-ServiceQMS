...
public class ExcelExportUtil2 {
    ...
    private static final Logger log = LoggerFactory.getLogger(ExcelExportUtil2.class);
...
//        ParamFontStyle2.setColor(HSSFColor.BLUE.index);   //设置字体颜色 (蓝色)
    ...
    public static void createBarChart(SXSSFSheet sheet, List<List<Object>> sheetDataList, List<Object> heads, ChartEntity chartEntity, List<List<Object>> barChartsheetDataList) {
        ...
        // 创建图表锚点（修正参数顺序：col1, row1, col2, row2）
    ...
    public byte[] exportExportNew(SXSSFWorkbook wb, HttpServletRequest request, HttpServletResponse response,
                                  ...
                                  List<DfProcessProjectConfig> dplist, List<NewImageRequest> projects, List<DfProject> activeProjects) throws IOException {
        /*
         ************************************************************************************
         */
        //第二个工作表
        SXSSFSheet sheet = wb.createSheet("各工序直通率-抽检良率");
        XSSFCellStyle cellParamStyle4 = null;
         ...
         */
        //第三个工作表
        SXSSFSheet sheet3 = wb.createSheet("各工序汇总TOP10不良");

        //全工序
      //  List<DfQmsIpqcWaigTotalVo> testV0Listall = getDfQmsIpqcWaigTotalVos3all(totalShiftList);
        // 表格
//        Map<String, Integer> centerTable1 = new HashMap<>();
//        if (testV0Listall.size() > 0) {
//            centerTable1 = createSheet3Table(cellParamStyle4, sheet3, 0, 0, testV0Listall);
//            createCenterChar(testV0Listall, sheet3, centerTable1.get("col") + 2);
//        }
...
//            Map<String, Integer> centerTable2 = createSheet3Table(cellParamStyle4, sheet3,0, 0, testV0List4101);
//            createCenterChar(testV0List4101, sheet3, centerTable2.get("col") + 2);
//        }
...
//         */
//        //第四个工作表
//        List<DfQmsIpqcWaigTotalVo> testV0List4 = getQmsIpqcWaigTotalVos(activeProjects,dplist);
//
...
//
//
//        SXSSFSheet sheet4 = wb.createSheet("各工序TOP5机台-各机台TOP不良");
//        Map<String, Integer> leftTable = new HashMap<>();
//        if (testV0List4.size() > 0) {
//            leftTable = createLeftTable(cellParamStyle4, sheet4, 0, 0, testV0List4);
//            createLeftChar(testV0List4, sheet4, leftTable.get("col"));
//        }
//        Map<String, Integer> centerTable = new HashMap<>();
//        int Rrow = 0;
//        // 中间表格
//        if (testV0List410.size() > 0) {
//            centerTable = createCenterTable(cellParamStyle4, sheet4, 0, leftTable.get("col") + 1, testV0List410);
//            Rrow = createCenterChar(testV0List410, sheet4, centerTable.get("col"));
//        }
//
//        if (testV0List411.size() > 0) {
//            Map<String, Integer> bottomTable = createBottomTable(cellParamStyle4, sheet4, centerTable.get("row") + 1, leftTable.get("col") + 3, testV0List411);
//            createBottomChar(testV0List411, sheet4, Rrow, centerTable.get("col") + 10);
//        }
...
//        SXSSFSheet sheet5 = wb.createSheet("各工序开机率-批通情况");
//
//
//        // 设置列宽
//        sheet5.setColumnWidth(0, 20 * 256);
//        sheet5.setColumnWidth(1, 20 * 256);
//        sheet5.setColumnWidth(2, 20 * 256);
//        sheet5.setColumnWidth(3, 20 * 256);
//        sheet5.setColumnWidth(4, 20 * 256);
//        sheet5.setColumnWidth(5, 20 * 256);
//
//        List<Excel2Data> datas5 = getDataList(totalShiftList);
//
//        if (datas5.size() > 0) {
//            createExcel(cellParamStyle4, sheet5, datas5);
//        }
//
//
//        /*
...
//        // 初始化一个二维列表，用于存储良率数据
//        if (datas.size() > 0) {
//            Sheet6(dplist, datas, sheet6, wb, 0, cellParamStyle);
//        }

        // 导出数据
        try {
            String name = generateFileName("工厂外观明细数据");
...
//                ));
     ...
     *
     * @param sheet       当前工作表
     * @param base64Image 图片的 Base64 数据
     * @param rowNumber   当前行号
     * @param label       图片标题
     * @return 插入图片后的下一行行号
     * @throws Exception 异常处理
     */
    private int insertImageWithLabel(Sheet sheet, String base64Image, int rowNumber, String label) {
        try {
            if (base64Image == null || base64Image.isEmpty()) {
                return rowNumber; // 如果图片为空，直接返回当前行号
            }

            // 创建标题行
            Row labelRow = sheet.getRow(rowNumber++);
            Cell labelCell = labelRow.createCell(35); // 标题放在 A 列
            labelCell.setCellValue(label);
            ...
            // 设置图片位置：图片放在标题的下一行
            anchor.setCol1(35); // 图片在 A 列
            anchor.setRow1(rowNumber); // 图片在标题行的下一行
            Picture picture = drawing.createPicture(anchor, pictureIdx);
            picture.resize(); // 自动调整图片大小

            // 插入图片后的下一行
            return rowNumber + 10; // 可根据图片高度调整行间距
        } catch (IllegalArgumentException e) {
            System.err.println("图片 Base64 解码失败，跳过当前图片: " + e.getMessage());
        } catch (Exception e) {
            System.err.println("处理图片时发生未知异常，跳过当前图片: " + e.getMessage());
        }
        // 如果发生任何异常，返回当前行号并继续
        return rowNumber;
    }
    ...
    private static void createExcel(XSSFCellStyle cellParamStyle4, SXSSFSheet sheet, List<Excel2Data> datas) {
        if (CollectionUtils.isEmpty(datas)) {
            return;
        }
        AtomicInteger lastCellIndex = new AtomicInteger();
        datas.forEach(item -> {
            if (CollectionUtils.isNotEmpty(item.getBbDataList())) {
                item.getBbDataList().forEach(v -> {
                    if (CollectionUtils.isNotEmpty(v.getInfoDataList())) {
                        v.getInfoDataList().forEach(v2 -> {
                            if (CollectionUtils.isNotEmpty(v2.getDays())) {
                                int size = v2.getDays().size();
                                if (lastCellIndex.get() < size) {
                                    lastCellIndex.set(size);
                                }
                            }
        ...
        int cellIndex = createBB(cellParamStyle4, sheet, datas);
        if (lastCellIndex.get() > 0) {
            cellIndex = lastCellIndex.get() + 3;
        }
        createYB(cellParamStyle4, sheet, datas, cellIndex);


    }


    private static int createYB(XSSFCellStyle cellParamStyle4, SXSSFSheet sheet, List<Excel2Data> datas, int cellIndex) {
        int chat = 0;
        int row = 0;
        ...
        for (int i = 0; i < datas.size(); i++) {
            ...
            if (datas.get(i) != null) {
                if (datas.get(i).getYbDataList() == null) {
                    continue;
                }
            }

            int cellEndIndex = datas.get(i).getYbDataList().get(0).getInfoDataList().get(0).getDays().size();
            endCellIndex = startCellIndex + cellEndIndex + 3;
            Excel2Data excel2Data = datas.get(i);
            String title = String.format("%s %s %s Total", "夜班", excel2Data.getProjectName(), excel2Data.getProcess());
            chat = createChat(sheet, datas.get(i).getYbDataList(), chat, startCellIndex, endCellIndex + startCellIndex, title);
            Map<String, Integer> table = createTableYB(cellParamStyle4, sheet, datas.get(i), chat, startCellIndex);
            row = table.get("lastRowNum");
            cellIndex = table.get("lastCellNum");
        }
        return cellIndex;
    }

    private static int createBB(XSSFCellStyle cellParamStyle4, SXSSFSheet sheet, List<Excel2Data> datas) {
        int chat = 0;
        int row = 0;
        int cellIndex = 0;
        int startCellIndex = 0;
        ...
        for (int i = 0; i < datas.size(); i++) {
            ...
            if (datas.get(i) != null) {
                if (datas.get(i).getBbDataList() == null) {
                    continue;
                }

            }
            int cellEndIndex = datas.get(i).getBbDataList().get(0).getInfoDataList().get(0).getDays().size();
            endCellIndex = cellEndIndex + 3;
            Excel2Data excel2Data = datas.get(i);
            String title = String.format("%s %s %s Total", "白班-", excel2Data.getProjectName(), excel2Data.getProcess());
            chat = createChat(sheet, datas.get(i).getBbDataList(), chat, startCellIndex, endCellIndex + startCellIndex, title);
            Map<String, Integer> table = createTableBB(cellParamStyle4, sheet, datas.get(i), chat);
            row = table.get("lastRowNum");
            cellIndex = table.get("lastCellNum");
        }
        return cellIndex;
    ...
    private static Map<String, Integer> createTableBB(XSSFCellStyle cellParamStyle4, Sheet sheet, Excel2Data excel2Data, int i) {
        int lastRowNum = 0;
        int lastCellNum = 0;
        HashMap<String, Integer> map = new HashMap<>();
        Row heard = sheet.getRow(i);
        if (heard == null) {
            heard = sheet.createRow(i);
        }
        String[] titles = {"项目", "工序", "日期"};
        for (int i2 = 0; i2 < titles.length; i2++) {
            Cell cell = heard.createCell(i2);
            cell.setCellValue(titles[i2]);
            cell.setCellStyle(cellParamStyle4);
        }


        for (int i1 = 0; i1 < titles.length; i1++) {
            Row row = sheet.getRow(heard.getRowNum() + 1);
            if (row == null) {
                row = sheet.createRow(heard.getRowNum() + 1);
            }
            if (i1 == 0) {
                Cell cell = row.createCell(i1);
                cell.setCellValue(excel2Data.getProjectName());
                cell.setCellStyle(cellParamStyle4);
            }
            if (i1 == 1) {
                Cell cell = row.createCell(i1);
                cell.setCellValue(excel2Data.getProcess());
                cell.setCellStyle(cellParamStyle4);

            }
            if (i1 == 2) {

                List<PaiBanData> bbDataList = excel2Data.getBbDataList();
                for (int i2 = 0; i2 < bbDataList.size(); i2++) {
                    PaiBanData paiBanData = bbDataList.get(i2);
                    if (paiBanData.getName().equals("白班")) {
                        List<InfoData> infoDataList = paiBanData.getInfoDataList();
                        for (int i3 = 0; i3 < infoDataList.size(); i3++) {
                            InfoData infoData = infoDataList.get(i3);
                            if (i3 == 0) {
                                Cell cell = row.createCell(i1);
                                cell.setCellValue(infoData.getName());
                                cell.setCellStyle(cellParamStyle4);

                                List<DayData> days = infoData.getDays();
                                for (int i4 = 0; i4 < days.size(); i4++) {
                                    Cell cell1 = row.createCell(i1 + i4 + 1);
                                    cell1.setCellValue(String.format("%.2f%%", days.get(i4).getNgl() * 100));
                                    cell1.setCellStyle(cellParamStyle4);

                                    Cell cell2 = heard.createCell(i1 + i4 + 1);
                                    cell2.setCellValue(infoData.getDays().get(i4).getDay());
                                    cell2.setCellStyle(cellParamStyle4);

                                }
                            } else {
                                Row row1 = sheet.getRow(row.getRowNum() + i3);
                                if (row1 == null) {
                                    row1 = sheet.createRow(row.getRowNum() + i3);
                                }
                                ...
                                List<DayData> days = infoData.getDays();
                                for (int i4 = 0; i4 < days.size(); i4++) {
                                    Cell cell1 = row1.createCell(i1 + i4 + 1);
                                    cell1.setCellValue(String.format("%.2f%%", days.get(i4).getNgl() * 100));
                                    cell1.setCellStyle(cellParamStyle4);
                                    ...
                                    lastRowNum = row1.getRowNum();
                                    lastCellNum = cell2.getColumnIndex();
                                }
                            }
                        }
                    }
        ...
        for (int i2 = 0; i2 < titles.length; i2++) {
            if (i2 < 2) {
                if (lastRowNum == 0) {
                    lastRowNum = heard.getRowNum() + 2;
                }
                sheet.addMergedRegion(new CellRangeAddress(heard.getRowNum() + 1, lastRowNum, i2, i2));
            }
        }


        map.put("lastRowNum", lastRowNum);
        map.put("lastCellNum", lastCellNum);
        return map;


    }

    private static Map<String, Integer> createTableYB(XSSFCellStyle cellParamStyle4, Sheet sheet, Excel2Data excel2Data, int i, int startCellIndex) {
        int lastRowNum = 0;
        int lastCellNum = 0;
        HashMap<String, Integer> map = new HashMap<>();
        Row heard = sheet.getRow(i);
        ...
        String[] titles = {"项目", "工序", "日期"};
        for (int i2 = 0; i2 < titles.length; i2++) {
            Cell cell = heard.createCell(startCellIndex + i2);
            cell.setCellValue(titles[i2]);
            cell.setCellStyle(cellParamStyle4);
        }

        // 如果bbDataList为空，直接返回，不继续处理
        List<PaiBanData> bbDataList = excel2Data.getYbDataList();
        if (CollectionUtils.isEmpty(bbDataList)) {
            // 如果数据为空，可以在这里进行日志记录或其他必要操作
            map.put("lastRowNum", heard.getRowNum());
            map.put("lastCellNum", startCellIndex + titles.length - 1);
            return map;
        }
        ...
        for (int i1 = 0; i1 < titles.length; i1++) {
            ...
            if (i1 == 0) {
                Cell cell = row.createCell(startCellIndex + i1);
                cell.setCellValue(excel2Data.getProjectName());
                cell.setCellStyle(cellParamStyle4);
            }
            if (i1 == 1) {
            ...
            if (i1 == 2) {
                ...
                for (int i2 = 0; i2 < bbDataList.size(); i2++) {
                    ...
                    if (paiBanData.getName().equals("夜班")) {
                        List<InfoData> infoDataList = paiBanData.getInfoDataList();
                        if (CollectionUtils.isEmpty(infoDataList)) {
                            // 如果infoDataList为空，可以记录日志并跳过后续处理
                            continue;
                        }
                        for (int i3 = 0; i3 < infoDataList.size(); i3++) {
                            InfoData infoData = infoDataList.get(i3);
                            if (i3 == 0) {
                                ...
                                List<DayData> days = infoData.getDays();
                                for (int i4 = 0; i4 < days.size(); i4++) {
                                    Cell cell1 = row.createCell(startCellIndex + i1 + i4 + 1);
                                    cell1.setCellValue(String.format("%.2f%%", days.get(i4).getNgl() * 100));
                                    cell1.setCellStyle(cellParamStyle4);
                                    Cell cell2 = heard.createCell(startCellIndex + i1 + i4 + 1);
                                    cell2.setCellValue(infoData.getDays().get(i4).getDay());
                                    cell2.setCellStyle(cellParamStyle4);
                                }
                            } else {
                                ...
                                Cell cell = row1.createCell(startCellIndex + i1);
                                cell.setCellValue(infoData.getName());
                                cell.setCellStyle(cellParamStyle4);

                                List<DayData> days = infoData.getDays();
                                for (int i4 = 0; i4 < days.size(); i4++) {
                                    Cell cell1 = row1.createCell(startCellIndex + i1 + i4 + 1);
                                    cell1.setCellValue(String.format("%.2f%%", days.get(i4).getNgl() * 100));
                                    cell1.setCellStyle(cellParamStyle4);
        ...
        for (int i2 = 0; i2 < titles.length; i2++) {
            if (i2 < 2) {
                ...
                sheet.addMergedRegion(new CellRangeAddress(heard.getRowNum() + 1, lastRowNum, startCellIndex + i2, startCellIndex + i2));
            }
        }
    ...
    private static int createChat(SXSSFSheet sheet, List<PaiBanData> bbDataList, int startRow, int startCellIndex,
                                  int endCellIndex, String title) {
        sheet.createDrawingPatriarch();
        XSSFDrawing drawing = sheet.getDrawingPatriarch();

        // Adjust startRow if necessary
        if (startRow != 0) {
            startRow = startRow + 1;
        }

        // Create chart anchor (positioning)
        XSSFClientAnchor anchor = drawing.createAnchor(0, 0, 0, 0, startCellIndex, startRow, endCellIndex, startRow + 12);
        XSSFChart chart = drawing.createChart(anchor);

        // Set chart title
        chart.setTitleText(title);  // Set your title here
        chart.setTitleOverlay(false);  // Ensure title is not overlaying the chart
        ...
        bottomAxis.setTitle("");  // Optionally set the title for X-axis

        // Create Y-axis (Value Axis)
        XDDFValueAxis leftAxis = chart.createValueAxis(AxisPosition.LEFT);
        leftAxis.setTitle(null);  // Explicitly remove the Y-axis title

        // Sample X-axis data
        String[] xTitleData = new String[]{"10.01", "10.02", "10.03", "10.04", "10.05", "10.06", "10.07"};
        List<InfoData> infoDataList = bbDataList.get(0).getInfoDataList();  // For simplicity, just use the first item

        // Adjust X-axis titles if there are days data
        for (int bb_index = 0; bb_index < bbDataList.size(); bb_index++) {
            for (int info_index = 0; info_index < infoDataList.size(); info_index++) {
                List<DayData> days = infoDataList.get(info_index).getDays();
                xTitleData = days.stream().map(item -> item.getDay()).toArray(String[]::new);
                break; // Just take the first day data for simplicity
            }
        }
        ...
        for (int i2 = 0; i2 < infoDataList.size(); i2++) {
//            Double[] xData1 = infoDataList.get(i2).getDays().stream()
//                    .map(item -> item != null && item.getNgl() != 0.00 ? item.getNgl() : 0.0)
//                    .toArray(Double[]::new);

            Double[] xData1 = infoDataList.get(i2).getDays().stream()
                    .map(item -> {
                        double value = (item != null && item.getNgl() != 0.00) ? item.getNgl() * 100 : 0.0;
            ...
            // Set the legend title (series name)
            series.setTitle(infoDataList.get(i2).getName(), null);

            // Set line smoothness: true for smooth line, false for straight line
            series.setSmooth(false);
    ...
    private List<Excel2Data> getDataList(List<DfQmsIpqcWaigTotal> totalShiftList5) {
        ArrayList<Excel2Data> excel2DataList = dogetbaiban(totalShiftList5);

        return excel2DataList;
    }


    private ArrayList<Excel2Data> dogetbaiban(List<DfQmsIpqcWaigTotal> totalShiftList5) {
        ArrayList<Excel2Data> resultList = new ArrayList<>();


        List<DfQmsIpqcWaigTotal> sortedTotalShiftList = totalShiftList5.stream()
                .sorted(Comparator.comparing(DfQmsIpqcWaigTotal::getSort))  // 按 firstCode 排序
                .collect(Collectors.toList());

        Map<String, List<DfQmsIpqcWaigTotal>> groupedData = sortedTotalShiftList.stream()
                .collect(Collectors.groupingBy(
        ...
        DfQmsIpqcWaigTotalService service = BeanUtils.getBean(DfQmsIpqcWaigTotalServiceImpl.class);

        // 获取日期范围
        List<LocalDate> formattedDates = getLastNineDays(startDate, endDate);
        formattedDates.sort(Comparator.naturalOrder());
        String startTimes = String.valueOf(formattedDates.get(0));
        String endTimes = String.valueOf(formattedDates.get(formattedDates.size() - 1));

        // 一次性获取所有白班和夜班数据（不再限制fSeq和fBigpro）
        List<DfQmsIpqcWaigTotal> allDayShift = service.getOptimizedDataWithCustomExecutor(
                startTimes, endTimes, "0", "11", null, null
        );
        List<DfQmsIpqcWaigTotal> allNightShift = service.getOptimizedDataWithCustomExecutor(
                startTimes, endTimes, "12", "23", null, null
        );

        // 构建快速查询缓存（分组键 -> 数据列表）
        Map<String, List<DfQmsIpqcWaigTotal>> dayShiftCache = allDayShift.stream()
                .collect(Collectors.groupingBy(
                        item -> String.join("_", item.getfBigpro(), item.getfSeq())
                ));
        Map<String, List<DfQmsIpqcWaigTotal>> nightShiftCache = allNightShift.stream()
                .collect(Collectors.groupingBy(
                        item -> String.join("_", item.getfBigpro(), item.getfSeq())
                ));

        // 处理每个分组
        for (Map.Entry<String, List<DfQmsIpqcWaigTotal>> entry : groupedData.entrySet()) {
            String key = entry.getKey();
            String[] keys = key.split("_");
            String fBigpro = keys[0];
            String fSeq = keys[1];

            Excel2Data excel2Data = new Excel2Data();
            excel2Data.setProjectName(fBigpro);
            excel2Data.setProcess(fSeq);

            // 从缓存获取数据（避免重复查询）
            List<DfQmsIpqcWaigTotal> blist = dayShiftCache.getOrDefault(key, Collections.emptyList());
            List<DfQmsIpqcWaigTotal> wlist = nightShiftCache.getOrDefault(key, Collections.emptyList());

            PaiBanData bbData = createPaiBanData("白班", blist, formattedDates);
            PaiBanData ybData = createPaiBanData("夜班", wlist, formattedDates);

            excel2Data.setBbDataList(Collections.singletonList(bbData));
            excel2Data.setYbDataList(Collections.singletonList(ybData));

            resultList.add(excel2Data);
        }
        return resultList;
    }


    private PaiBanData createPaiBanData(String name, List<DfQmsIpqcWaigTotal> dataList, List<LocalDate> formattedDates) {
        PaiBanData paiBanData = new PaiBanData();
        paiBanData.setName(name);

        ArrayList<InfoData> infoDataList = new ArrayList<>();
        InfoData faiInfoData = new InfoData();
        faiInfoData.setName("FAI通过率");
        InfoData cpkInfoData = new InfoData();
        cpkInfoData.setName("CPK通过率");

        ArrayList<DayData> faiDayDataList = new ArrayList<>();
        ArrayList<DayData> cpkDayDataList = new ArrayList<>();

        for (LocalDate date : formattedDates) {
            String formattedDateString = date.toString();
            Optional<DfQmsIpqcWaigTotal> optionalData = dataList.stream()
                    .filter(dfQms -> formattedDateString.equals(dfQms.getDate()))
                    .findFirst();

            DayData faiDayData = new DayData();
            faiDayData.setDay(formattedDateString);
            faiDayData.setNgl(optionalData.map(DfQmsIpqcWaigTotal::getFaiStartupRate).orElse(0.0));
            faiDayDataList.add(faiDayData);

            DayData cpkDayData = new DayData();
            cpkDayData.setDay(formattedDateString);
            cpkDayData.setNgl(optionalData.map(DfQmsIpqcWaigTotal::getCpkPassRate).orElse(0.0));
            cpkDayDataList.add(cpkDayData);
        }

        faiInfoData.setDays(faiDayDataList);
        cpkInfoData.setDays(cpkDayDataList);
        infoDataList.add(faiInfoData);
        infoDataList.add(cpkInfoData);
        paiBanData.setInfoDataList(infoDataList);
    ...
    public static <
            T> java.util.function.Predicate<T> distinctByKey(java.util.function.Function<? super T, ?> keyExtractor) {
        Set<Object> seen = new HashSet<>();
        return t -> seen.add(keyExtractor.apply(t));
    }


    private List<DfQmsIpqcWaigTotalVo> getQmsIpqcWaigTotalVos
            (List<DfProject> totalShiftList,List<DfProcessProjectConfig> dplist) {
...
//    private List<DfQmsIpqcWaigTotalVo> getDfQmsIpqcWaigTotalVos3all(List<DfQmsIpqcWaigTotal> totalShiftList) {
//
//
//        List<DfQmsIpqcWaigTotal> sortedTotalShiftList = totalShiftList.stream()
//                .sorted(Comparator.comparing(DfQmsIpqcWaigTotal::getSort))  // 按 firstCode 排序
//                .collect(Collectors.toList());
//
//        Map<String, List<DfQmsIpqcWaigTotal>> groupedData = sortedTotalShiftList.stream()
//                .collect(Collectors.groupingBy(
//                        item -> String.join("_", item.getfBigpro()),
//                        LinkedHashMap::new,  // 使用 LinkedHashMap 保持插入顺序
//                        Collectors.toList()));
//
//        List<DfQmsIpqcWaigTotalVo> testV0List410 = new ArrayList<>();
...
//
//            // 定义 banbie 和数据列表
//            List<Banbie> banbies10 = createBanbies(startDate, endDate, fBigpro, fSeq, type, null);
//
//            // 设置结果
//            vo.setBbList(banbies10);
//            testV0List410.add(vo);
...
/*    private static Map<String, Integer> createBottomTable(Sheet sheet, int firstRow, int firstCol, List<
        ...
        for (DfQmsIpqcWaigTotalVo table : datas) {
            ...
            String[] headers = {"日期", "厂别", "项目", "工序", "机台", "不良项目", "班别"};
            for (int i = 0; i < headers.length; i++) {
                Cell cell = headerRow.createCell(firstCol + 2 + i);
                cell.setCellValue(headers[i]);
            }
            totalRow++;
            List<NotProject> bbList = table.getNotProjectList();
            ...
            for (NotProject bb : bbList) {
                for (int i = 0; i < headers.length; i++) {
                    if (i + firstCol + 2 == firstCol + 2) {
                        Cell cell = row.createCell(firstCol + 2 + i);
                        cell.setCellValue(table.getData());
                    }
                    if (i + firstCol + 2 == firstCol + 2 + 1) {
                        Cell cell = row.createCell(firstCol + 2 + i);
                        cell.setCellValue(table.getfFac());
                    }
                    if (i + firstCol + 2 == firstCol + 2 + 2) {
                        Cell cell = row.createCell(firstCol + 2 + i);
                        cell.setCellValue(table.getfBigpro());
                    }
                    if (i + firstCol + 2 == firstCol + 2 + 3) {
                        Cell cell = row.createCell(firstCol + 2 + i);
                        cell.setCellValue(table.getFseq());
                    }
                    if (i + firstCol + 2 == firstCol + 2 + 4) {
                        Cell cell = row.createCell(firstCol + 2 + i);
                        cell.setCellValue(table.getFmac());
                    }
                    if (i + firstCol + 2 == firstCol + 2 + 5) {
                            ...
                            if (i1 == 0) {
                                ...
                                for (int i2 = 0; i2 < banbieList.size(); i2++) {
                                    ...
                                    for (int i3 = 0; i3 < dayDatas.size(); i3++) {
                                        ...
                                        cell4.setCellValue(dayDatas.get(i3).getNgl());
                                    }
                                    fisRow = row1;
                                ...
                                Row row1 = sheet.getRow(fisRow.getRowNum() + 1);
                                if (row1 == null) {
                                    row1 = sheet.createRow(fisRow.getRowNum() + 1);
                                }
                                Cell cell = row1.createCell(firstCol + 2 + i);
                                cell.setCellValue(notProject.getName());
                                for (int i2 = 0; i2 < banbieList.size(); i2++) {
                                    Row row2 = sheet.getRow(row1.getRowNum() + i2);
                                    if (row2 == null) {
                                        row2 = sheet.createRow(row1.getRowNum() + i2);
                                    }
                                    Cell cell1 = row2.createCell(cell.getColumnIndex() + 1);
                                    cell1.setCellValue(banbieList.get(i2).getName());
                                    if (!banbieList.get(i2).getName().equals("合计")) {
                                        Cell cell2 = row2.createCell(cell1.getColumnIndex() + 1);
                                        cell2.setCellValue(banbieList.get(i2).getLable());
                                    }


                                    totalRow = row2.getRowNum() + 1;
                                    fisRow = row2;
                                    List<DayData> dayDatas = banbieList.get(i2).getDayDatas();
                                    for (int i3 = 0; i3 < dayDatas.size(); i3++) {

                                        Cell cell4 = row2.createCell(cell1.getColumnIndex() + i3 + 2);
                                        cell4.setCellValue(dayDatas.get(i3).getNgl());
                                        map.put("col", cell4.getColumnIndex());
                                    }
                                }
                            }
                        }
            ...
            for (int i = 0; i < headers.length + 1; i++) {
                if (i == 0 || i == 1 || i == 2 || i == 3 || i == 4||i==5) {
                    ...
                    sheet.addMergedRegion(new CellRangeAddress(startRow, totalRow, firstCol + i + 2, firstCol + i + 2));
                }
                if (i == 6) {
//                    int lastIndex = 0;
...
//                            startRow = lastIndex;
//                        }
                    for (int i2 = 0; i2 < table.getNotProjectList().size(); i2++) {

                   //     sheet.addMergedRegion(new CellRangeAddress(startRow + (i2 * 3), startRow + 2 + (i2 * 3), firstCol + i + 2, firstCol + i + 2));
                        sheet.addMergedRegion(new CellRangeAddress(startRow + 2 + (i2 * 3), startRow + 2 + (i2 * 3), firstCol + i + 3, firstCol + i + 4));
//                            if(i2==datas.get(i1).getNotProjectList().size()-1){
//                                lastIndex= startRow + 2 + (i2 * 3)+1;
//                            }
...
//                    }
 ...
 /*   private static List<DfQmsIpqcWaigTotalVo> getDfQmsIpqcWaigTotalVoss
            (ArrayList<Map<String, DfQmsIpqcWaigTotal>> dpmap10, ArrayList<String> delect) {
        ...
        ArrayList<DayData> BbjtData10 = new ArrayList<>();
        ArrayList<DayData> wBjtData10 = new ArrayList<>();
        ArrayList<DayData> hZjtData10 = new ArrayList<>();
        ArrayList<NotProject> notProjects = new ArrayList<NotProject>();
        ...
        zh10.forEach((key, value) -> {
            ...
            if (zh10.size() > 0 && ro10.containsKey(key)) {
                ...
                banbie310.setLable("NG率");
                DayData dayData = new DayData();
                dayData.setDay(v.getDate());
                dayData.setNgl(v.getNgRate());
                BbjtData10.add(dayData);
                banbie310.setDayDatas(BbjtData10);
                ...
                banbie310.setLable("NG率");
                DayData dayData = new DayData();
                dayData.setDay(zh10.get(key).getDate());
                dayData.setNgl(0);
                BbjtData10.add(dayData);
                banbie310.setDayDatas(BbjtData10);
                banbies10.add(banbie310);
            ...
            if (ng10.containsKey(key)) {
                ...
                banbie410.setLable("NG率");
                DayData dayData = new DayData();
                dayData.setDay(v.getDate());
                dayData.setNgl(v.getNgRate());
                wBjtData10.add(dayData);
                banbie410.setDayDatas(wBjtData10);
            ...
            banbie210.setName("合计");
            banbie210.setLable("NG率");
            DayData dayData = new DayData();
            dayData.setDay(zh10.get(key).getDate());
            dayData.setNgl(zh10.get(key).getNgRate());
            hZjtData10.add(dayData);
            banbie210.setDayDatas(hZjtData10);
            banbies10.add(banbie210);
            NotProject notProject = new NotProject();
            notProject.setName(zh10.get(key).getfSort());
            notProject.setBanbieList(banbies10);
            notProjects.add(notProject);
            testv0110.setData(zh10.get(key).getDate());
            testv0110.setfFac(zh10.get(key).getfFac());
            testv0110.setfBigpro(zh10.get(key).getfBigpro());
            testv0110.setFseq(zh10.get(key).getfSeq());
            testv0110.setFmac(zh10.get(key).getfMac());
            testv0110.setNotProjectList(notProjects);
    ...
    private List<DfQmsIpqcWaigTotalVo> getDfQmsIpqcWaigTotalVoss(List<DfQmsIpqcWaigTotal> totalShiftList) {


//        Map<String, List<DfQmsIpqcWaigTotal>> groupedByDate = totalShiftList.stream()
//                .collect(Collectors.groupingBy(DfQmsIpqcWaigTotal::getDate));

        // 创建一个 List<String> 来存储最后九个日期
        ...
        // 将每个日期转换为字符串，并加入到 lastNineDates 中
        lastNineDates.addAll(formattedDates.stream()
                .map(date -> date.toString())  // 将 LocalDate 转换为 String
        ...
        Map<String, List<DfQmsIpqcWaigTotal>> groupedData = sortedTotalShiftList.stream()
                .collect(Collectors.groupingBy(
                        item -> String.join("_", item.getfFac(), item.getfBigpro(), item.getfSeq(), item.getfMac(), item.getfSort()),
                        LinkedHashMap::new,  // 使用 LinkedHashMap 保持插入顺序
                        Collectors.toList()));
        ...
        for (Map.Entry<String, List<DfQmsIpqcWaigTotal>> entry : groupedData.entrySet()) {
            ...
            vo.setfFac(keys[0]);
            vo.setfBigpro(keys[1]);
            vo.setFseq(keys[2]);
            vo.setFmac(keys[3]);
            vo.setFsort(keys[4]);

            // 检查 keys[4] 是否为空或 "null"
            boolean isKey4Invalid = keys.length < 5 || keys[4].isEmpty() || "null".equalsIgnoreCase(keys[4]);
            ...
            if (isKey4Invalid) {
                lastNineDates.forEach(e -> {
                    // 白班数据全零
                    DayData Bdata = new DayData();
                    Bdata.setDay(e);
                    Bdata.setSpotCheckCount(0);
                    Bdata.setNgNum(0);
                    Bdata.setNgl(0.0);
                    BbjtData10.add(Bdata);

                    // 晚班数据全零
                    DayData ydata = new DayData();
                    ydata.setDay(e);
                    ydata.setSpotCheckCount(0);
                    ydata.setNgNum(0);
                    ydata.setNgl(0.0);
                    wBjtData10.add(ydata);

                    // 合计数据全零
                    DayData hdata = new DayData();
                    hdata.setDay(e);
                    hdata.setSpotCheckCount(0);
                    hdata.setNgNum(0);
                    hdata.setNgl(0.0);
                    hZjtData10.add(hdata);
                });
            } else {


                lastNineDates.forEach(e -> {
                    List<DfQmsIpqcWaigTotal> dfmsq = groupedByFost.get(e);
                    if (dfmsq != null) {
                        Map<String, DayData> tempMap = new HashMap<>();
                        Map<String, DayData> ytempMap = new HashMap<>();
                        Map<String, DayData> htempMap = new HashMap<>();
                        dfmsq.forEach(f -> {
                            String isWhiteShift = f.getShift();
                            if ("白班".equals(isWhiteShift)) {
                                DayData day4 = new DayData();
                                day4.setDay(f.getDate());
                                day4.setSpotCheckCount(f.getSpotCheckCount());
                                day4.setNgNum(Integer.valueOf(f.getNgNum()));
                                day4.setNgl(Double.valueOf(String.format("%.2f", f.getNgRate() * 100)));
                                // 判断是否存在相同 fMac 的数据，存在则累加，否则添加
                                if (tempMap.containsKey(f.getDate())) {
                                    DayData existingData = tempMap.get(f.getDate());
                                    // 更新抽检数，确保不为负数
                                    int newSpotCheckCount = existingData.getSpotCheckCount() + f.getSpotCheckCount(); // 更新抽检数
                                    existingData.setSpotCheckCount(newSpotCheckCount); // 设置新的抽检数

                                    // 更新不良数，确保不为负数
                                    int newNgNum = existingData.getNgNum() + Integer.valueOf(f.getNgNum()); // 更新不良数
                                    existingData.setNgNum(newNgNum); // 设置新的不良数

                                    // 计算不良率，确保新的抽检数不为零
                                    if (newSpotCheckCount > 0) { // 判断抽检数是否大于零
                                        double ngRate = (double) newNgNum / newSpotCheckCount; // 计算不良率：$\text{ngRate} = \frac{\text{newNgNum}}{\text{newSpotCheckCount}}$
                                        existingData.setNgl(Double.valueOf(String.format("%.2f", ngRate * 100))); // 设置新的不良率
                                    } else {
                                        existingData.setNgl(0.0); // 如果抽检数为零，则不良率设置为 0
                                    }
                                    tempMap.put(f.getDate(), existingData);
                                } else {
                                    tempMap.put(f.getDate(), day4);
                                }
                                DayData hjdata = new DayData();
                                hjdata.setDay(f.getDate());
                                hjdata.setSpotCheckCount(0);
                                hjdata.setNgNum(0);
                                hjdata.setNgl(0);
                                ytempMap.put(f.getDate(), hjdata);
                            } else if ("晚班".equals(isWhiteShift)) {
                                DayData ydata = new DayData();
                                ydata.setDay(f.getDate());
                                // data.setfFmac(dfms.getfMac());
                                ydata.setSpotCheckCount(f.getSpotCheckCount());
                                ydata.setNgNum(Integer.valueOf(f.ngNum));
                                ydata.setNgl(Double.valueOf(String.format("%.2f", f.getNgRate() * 100)));
                                // 判断是否存在相同 fMac 的数据，存在则累加，否则添加
                                if (ytempMap.containsKey(f.getDate())) {
                                    DayData existingData = ytempMap.get(f.getDate());
                                    // 更新抽检数，确保不为负数
                                    int newSpotCheckCount = existingData.getSpotCheckCount() + f.getSpotCheckCount(); // 更新抽检数
                                    existingData.setSpotCheckCount(newSpotCheckCount); // 设置新的抽检数
                                    ...
                                    if (newSpotCheckCount > 0) { // 判断抽检数是否大于零
                                        ...
                                        existingData.setNgl(0.00); // 如果抽检数为零，则不良率设置为 0
                                    }
                                    ytempMap.put(f.getDate(), existingData);
                                } else {
                                    ytempMap.put(f.getDate(), ydata);
                                }

                                DayData bdata = new DayData();
                                bdata.setDay(f.getDate());
                                bdata.setSpotCheckCount(0);
                                bdata.setNgNum(0);
                                bdata.setNgl(0);
                                tempMap.put(f.getDate(), bdata);
                            }

                            DayData data = new DayData();
                            data.setDay(f.getDate());
                            // data.setfFmac(dfms.getfMac());
                            data.setSpotCheckCount(f.getSpotCheckCount());
                            data.setNgNum(Integer.valueOf(f.getNgNum()));
                            data.setNgl(Double.valueOf(String.format("%.2f", f.getNgRate() * 100)));
                            if (htempMap.containsKey(f.getDate())) {

                                DayData existingData = htempMap.get(f.getDate());
                                // 更新抽检数，确保不为负数
                                int newSpotCheckCount = existingData.getSpotCheckCount() + f.getSpotCheckCount(); // 更新抽检数
                                existingData.setSpotCheckCount(newSpotCheckCount); // 设置新的抽检数
                                ...
                                htempMap.put(f.getDate(), existingData);
                            } else {
                                htempMap.put(f.getDate(), data);
                            }


                        });
                        tempMap.forEach((fMac, aggregatedData) -> {
                            BbjtData10.add(aggregatedData);
                        });
                        ytempMap.forEach((fMac, aggregatedData) -> {
                            wBjtData10.add(aggregatedData);
                        });

                        htempMap.forEach((fMac, aggregatedData) -> {
                            hZjtData10.add(aggregatedData);
                        });
                    } else {
                        DayData Bdata = new DayData();
                        Bdata.setDay(e);
            ...
            banbies10.add(banbie210);
            NotProject notProject = new NotProject();
            notProject.setName(vo.getFsort());
            notProject.setBanbieList(banbies10);
            notProjects.add(notProject);
            vo.setNotProjectList(notProjects);
            // vo.setBbList(banbies10);
            testV0List410.add(vo);

        }
/*        List<DfQmsIpqcWaigTotalVo> testV0List410Sorted = testV0List410.stream()
                .sorted(Comparator.comparing(DfQmsIpqcWaigTotalVo::getData).reversed())
                .collect(Collectors.toList());*/
        return testV0List410;
    }


    private static void createBottomChar(List<DfQmsIpqcWaigTotalVo> bottomTables, SXSSFSheet sheet, int row, Integer
            endcol) {
        // 用来动态更新行号，以避免多个图表重叠
        int currentRow = row;
        ...
        for (int i = 0; i < bottomTables.size(); i++) {
            ...
            for (int i1 = 0; i1 < notProjectList.size(); i1++) {
                ...
                chartTitleBuilder
                        ...
                        .append(" ")
                        .append(table1.getFmac())
                        .append(" ")
                        .append(notProject.getName());
    ...
    private static int createCenterChar(List<DfQmsIpqcWaigTotalVo> centerTables, SXSSFSheet sheet, Integer endcol) {
        ...
        for (int i = 0; i < centerTables.size(); i++) {
            ...
            for (int i1 = 0; i1 < bbList.size(); i1++) {
                ...
                // 使用 StringBuilder 更高效地构建标题
                StringBuilder chartTitleBuilder = new StringBuilder();
                String type = table1.getFmac();
                if (StringUtils.isEmpty(table1.getFmac())) {
                    chartTitleBuilder
                            .append(table1.getfFac())
                            .append(" ")
                ...
                XSSFClientAnchor anchor = drawing.createAnchor(0, 0, 0, 0, endcol + 5, rowOffset, endcol + 10, rowOffset + 12);
    ...
    private static Map<String, Integer> createBottomTable(XSSFCellStyle cellParamStyle4, SXSSFSheet sheet, int firstRow, int firstCol, List<DfQmsIpqcWaigTotalVo> datas) {
        ...
        for (DfQmsIpqcWaigTotalVo table : datas) {
                ...
                for (int i = 0; i < headers.length; i++) {
                    ...
                    if (i + firstCol == firstCol + 5) {
                        ...
                        if (i1 == 0) {
                            ...
                            Row row1 = sheet.getRow(row.getRowNum());
                            if (row1 == null) {
                                row1 = sheet.createRow(row.getRowNum());
                            }
                            Cell cell = row1.createCell(firstCol + i);
                            cell.setCellValue(notProject.getName());
                            cell.setCellStyle(cellParamStyle4);
            ...
            for (int i2 = 0; i2 < notProjectList.size(); i2++) {
                for (int i = 0; i < headers.length + 1; i++) {
                    if (i == 5) {
                        sheet.addMergedRegion(new CellRangeAddress(startRow + (i2 * 3), startRow + 2 + (i2 * 3), firstCol + i, firstCol + i));
                        sheet.addMergedRegion(new CellRangeAddress(startRow + 2 + (i2 * 3), startRow + 2 + (i2 * 3), firstCol + i + 1, firstCol + i + 2));
                        /*if(i2==0){
                            sheet.addMergedRegion(new CellRangeAddress(startRow + 3 + (i2 * 3), startRow + 3 + (i2 * 3), firstCol + i + 1, firstCol + i + 2));
                        }else{
                            sheet.addMergedRegion(new CellRangeAddress(startRow + 2 + (i2 * 3), startRow + 2 + (i2 * 3), firstCol + i + 1, firstCol + i + 2));
                        }*/
                    ...
                    if (i == 0 || i == 1 || i == 2 || i == 3 || i == 4) {
                        ...
                        sheet.addMergedRegion(new CellRangeAddress(headerStartRow, endIndex, firstCol + i, firstCol + i));
                    }
                }
            }
        }
...
/*private static void createCenterChar(List<DfQmsIpqcWaigTotalVo> centerTables, Sheet sheet, Integer endcol) {
    ...
    private static Map<String, Integer> createCenterTable(XSSFCellStyle cellParamStyle4, SXSSFSheet sheet, int firstRow, int firstCol, List<
            DfQmsIpqcWaigTotalVo> datas) {
        HashMap<String, Integer> map = new HashMap<>();
        ...
        for (DfQmsIpqcWaigTotalVo table : datas) {
            ...
            SXSSFRow headerRow = sheet.getRow(totalRow);
            if (headerRow == null) {
                headerRow = sheet.createRow(totalRow);
            }
            String[] headers = {"日期", "厂别", "项目", "工序", "机台", "班别"};
            for (int i = 0; i < headers.length; i++) {
                Cell cell = headerRow.createCell(firstCol + 2 + i);
                cell.setCellValue(headers[i]);
                cell.setCellStyle(cellParamStyle4);
            ...
            totalRow++;
            List<Banbie> bbList = table.getBbList();
            SXSSFRow row = sheet.getRow(totalRow);
            if (row == null) {
                row = sheet.createRow(totalRow);
            }
            totalRow++;
            ...
            for (Banbie bb : bbList) {
                for (int i = 0; i < headers.length; i++) {
                    ...
                    if (i + firstCol + 2 == firstCol + 2 + 5) {
                        int colIndex = i + firstCol + 2 + 1;
                        SXSSFRow bbRow = null;
                        if (bb.getName().equals("汇总")) {
                            bbRow = row;
                            Cell cell = bbRow.createCell(firstCol + 2 + i);
                            cell.setCellValue(bb.getName());
                            cell.setCellStyle(cellParamStyle4);
                            totalRow = totalRow + 2;
                            for (int j = 0; j < 4; j++) {
            ...
            for (int i = 0; i < headers.length; i++) {
                if (i != 5) {
                    sheet.addMergedRegion(new CellRangeAddress(startRow, totalRow - 1, firstCol + i + 2, firstCol + i + 2));
                } else {
                    sheet.addMergedRegion(new CellRangeAddress(startRow, totalRow - 9, firstCol + i + 2, firstCol + i + 2));
                    sheet.addMergedRegion(new CellRangeAddress(totalRow - 8, totalRow - 5, firstCol + i + 2, firstCol + i + 2));
                    sheet.addMergedRegion(new CellRangeAddress(totalRow - 4, totalRow - 1, firstCol + i + 2, firstCol + i + 2));
                }
            }
        }
        map.put("row", totalRow);
     ...
     * @param col
     */
    private static void createLeftChar(List<DfQmsIpqcWaigTotalVo> leftTables, SXSSFSheet sheet, int col) {
        // 用来跟踪当前可用的起始行
...
/*                // 图例
    ...
    private void writeRowData(SXSSFSheet sheet, int rowIndex, Map<String, Object> map, XSSFCellStyle style) {
        ...
        // 预先缓存 ddNum 和 sjNum 的值，避免重复调用 map.get()
        Long ddNum = null;
        Long sjNum = null;
        Object ddObj = map.get("ddNum");
        Object sjObj = map.get("sjNum");
        if (ddObj instanceof Number) {
            ddNum = ((Number) ddObj).longValue();
        }
        if (sjObj instanceof Number) {
            sjNum = ((Number) sjObj).longValue();
        }

        for (int j = 0; j < heardKey.length; j++) {
            SXSSFCell cell = row.createCell(j);
            // 统一设置单元格样式（如果后续需要特殊样式可另外处理）
            cell.setCellStyle(style);
            ...
            // 如果当前列是 ddNum 或 sjNum，根据业务需求可做额外处理
            if ("ddNum".equals(heardKey[j]) || "sjNum".equals(heardKey[j])) {
                // 示例逻辑：如果 ddNum 与 sjNum 均存在且不相等，可以选择设置不同样式
                if (ddNum != null && sjNum != null && !ddNum.equals(sjNum)) {
                    // 如果需要不同的样式（比如标记颜色），可以在这里设置：cell.setCellStyle(alternateStyle);
                    // 此处示例中保持原样
                }
            }

            // 设置单元格内容，假设 Strings.isEmpty() 是判空工具方法
            cell.setCellValue(Strings.isEmpty(value) ? "" : value);
        }
    }
...
//        ParamFontStyle2.setColor(HSSFColor.BLUE.index);   //设置字体颜色 (蓝色)
    ...
    private static Map<String, Integer> createLeftTable(XSSFCellStyle cellParamStyle4, SXSSFSheet sheet, int firstRow, int firstCol, List<
            DfQmsIpqcWaigTotalVo> datas) {
        HashMap<String, Integer> map = new HashMap<>();
        // 创建表头
        // 创建表头
...
//            }
            ...
            if (i == 0) {
                ...
                totalDirectRate1.add(String.format("%.2f%%", ShiftYieldRate * 100));
                ...
                if (ZTlist.size() > 0) {
                    ...
                    totalDirectRate1.add(String.format("%.2f%%", newDirectRate * 100)); // 更新直通率
                ...
                if (BBlist.size() > 0) {
                    ...
                    whiteDirectRate1.add(String.format("%.2f%%", BbDirectRate * 100));
                ...
                if (WBlist.size() > 0) {
                    ...
                    nightDirectRate1.add(String.format("%.2f%%", WbDirectRate * 100));
            ...
            totalShiftYieldRate1.add(String.format("%.2f%%", ShiftYieldRate * 100));
        }
    }

    public XSSFCellStyle createTitleCellStyle(SXSSFWorkbook wb) {
        // 使用XSSFWorkbook的样式和字体
