...
import com.ww.boengongye.mapper.DfSopControllerFileMapper;
import com.ww.boengongye.service.DfSopControllerFileService;
 ...
 */
@Service
public class DfSopControllerFileServiceImpl extends ServiceImpl<DfSopControllerFileMapper, DfSopControllerFile> implements DfSopControllerFileService {
    @Autowired
    com.ww.boengongye.mapper.DfSopControllerFileMapper DfSopControllerFileMapper;
...
//        Map<String, Object> someTitle = importUtil.readExcelSomeTitle();
        ...
        List<DfSopControllerFile> orderDetails = maps.stream().filter(Objects::nonNull).map(map -> {
            DfSopControllerFile tc = new DfSopControllerFile();
...
//            tc.set(map.get(""));
        ...
        if (orderDetails.size() > 0) {
            ...
            int i=1;
            for (DfSopControllerFile c : orderDetails) {
                ...
                DfSopControllerFileMapper.insert(c);
                i++;
            }
