 ...
 */
@Controller
@RequestMapping("/dfTemperature")
@CrossOrigin
@ResponseBody
@Api(tags = "温度表")
public class DfTemperatureController {
    @Autowired
    private DfTemperatureService dfTemperatureService;
		...
		for (DfDustMonitor entity:list){
			if ("NG".equals(entity.getCheckResult())){
				...
				if (lm==null||lm.size()==0){
					return new Result(200,"没有相关温度责任人数据，无法生成审批单");
				}
				//责任人编号
				...
				fd.setName("温度"+ aud.getQuestionName() + "_NG_" + checkTimeNow);
    ...
    @ApiOperation("获取温度最新数据")
    public Result getNowData(){
        QueryWrapper<DfTemperature> ew = new QueryWrapper<>();
        ew.orderByDesc("check_time")
            .last("limit 1");
        DfTemperature dfTemperature = dfTemperatureService.getOne(ew);
        return new Result(200,"获取温度最新数据成功",dfTemperature.getCheckValue());
    }

    @GetMapping("/getTemperatureNew")
    @ApiOperation("获取温度全部数据")
    public Result getTemperatureNew(@RequestParam String startDate , @RequestParam String endDate,String factory,String linebody,String project,String color){
        String startTime = startDate + " 00:00:00";
        String endTime = endDate + " 23:59:59";
        QueryWrapper<DfTemperature> ew = new QueryWrapper<>();
        ew.select("date_format(check_time, '%m-%d') time","avg(lsl) lsl","avg(usl) usl","avg(check_value) check_result")
                .between("check_time", startTime,endTime)
                .groupBy("time")
                .orderByDesc("time");
        List<Map<String,Object>> list = dfTemperatureService.listMaps(ew);
        ...
        for (Map<String,Object> one : list) {
            checkValue.add(one.get("check_result"));
            up.add(one.get("usl"));
            down.add(one.get("lsl"));
            date.add(one.get("time"));
        }
        QueryWrapper<DfTemperature> ew2 = new QueryWrapper<>();
        ew2.orderByDesc("check_time")
                .last("limit 1");
        DfTemperature dfTemperature = dfTemperatureService.getOne(ew2);
        map.put("checkValue",checkValue);
        map.put("up",up);
        map.put("down",down);
        map.put("date",date);
        map.put("newOne",dfTemperature.getCheckValue());
        return new Result(200,"获取温度最新数据成功",map);
    }

	@ApiOperation("新增或修改")
	@PostMapping("/saveOrUpdate")
	public Object saveOrUpdate(@RequestBody DfTemperature datas){
		if (null != datas.getId()) {
			if (dfTemperatureService.updateById(datas)) {
...
//            datas.setCreateTime(TimeUtil.getNowTimeByNormal());
			if (dfTemperatureService.save(datas)) {
				return new Result(200, "保存成功");
			} else {
				return new Result(500, "保存失败");
			}
	...
	public Object deleteById(@RequestParam  String id){
		if (dfTemperatureService.removeById(id)){
	...
	public Object conditionQuery(int page,int limit,String factoryArea,String factory,String checkPosition,String startDate,String endDate){
		Page<DfTemperature> pages = new Page<>(page, limit);
		QueryWrapper<DfTemperature> ew = new QueryWrapper<>();
		...
		IPage<DfTemperature> list = dfTemperatureService.page(pages, ew);
