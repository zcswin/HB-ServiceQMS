 ...
 */
@Controller
@RequestMapping("/dfFlowDrawingFile")
@ResponseBody
@CrossOrigin
public class DfFlowDrawingFileController {
    @Autowired
    com.ww.boengongye.service.DfFlowDrawingFileService DfFlowDrawingFileService;

    @Autowired
    private Environment env;

    private static final Logger logger = LoggerFactory.getLogger(DfFlowDrawingFileController.class);

    @RequestMapping(value = "/listAll")
    public Object listAll() {

        return new Result(0, "查询成功",DfFlowDrawingFileService.list());
    }



    @RequestMapping(value = "/saveOrUpdate", method = RequestMethod.POST)
    public Result save(@RequestBody DfFlowDrawingFile datas) {

        if (null != datas.getId()) {
            if (DfFlowDrawingFileService.updateById(datas)) {
...
//            datas.setCreateTime(TimeUtil.getNowTimeByNormal());
            if (DfFlowDrawingFileService.save(datas)) {
                return new Result(200, "保存成功");
            } else {
                return new Result(500, "保存失败");
            }
...
//        try {
        if (DfFlowDrawingFileService.removeById(id)) {
...
//        return new Result(500, "接口异常");
    ...
    public Result listAllByAdmin(int page ,int limit,String category,String factoryCode,String changeScope,String projectCode) {
//        try {
        Page<DfFlowDrawingFile> pages = new Page<DfFlowDrawingFile>(page, limit);
        QueryWrapper<DfFlowDrawingFile> ew=new QueryWrapper<DfFlowDrawingFile>();
        ...
        if(null!=changeScope&&!changeScope.equals("")) {
            ew.like("change_scope", changeScope);
        }
        if(null!=projectCode&&!projectCode.equals("")) {
            ew.like("project_code", projectCode);
        }
        ew.orderByDesc("create_time");
        IPage<DfFlowDrawingFile> list=DfFlowDrawingFileService.page(pages,ew);
...
//	        img.setArticleId(id);
            ...
            try {
                ImportExcelResult ter = DfFlowDrawingFileService.importOrder(env.getProperty("uploadPath") + "excel/" + fileName, file);
