...
public class ProcessConfigController {
    @Autowired
    com.ww.boengongye.service.ProcessConfigService ProcessConfigService;
    ...
    public Object listAll() {
        ...
        return new Result(0, "查询成功",ProcessConfigService.list());
    }



    @RequestMapping(value = "/saveOrUpdate", method = RequestMethod.POST)
    public Result save(@RequestBody ProcessConfig datas) {
//        try {
        Field[] f= ProcessConfig.class.getDeclaredFields();
        //给TAnnals对象赋值
        for(int i=0;i<f.length;i++){
            ...
            try{
                ...
                Method setMethod= ProcessConfig.class.getMethod("set"+methodName,String.class);
        ...
        if (null != datas.getId()) {
            if (ProcessConfigService.updateById(datas)) {
...
//            datas.setCreateTime(TimeUtil.getNowTimeByNormal());
            if (ProcessConfigService.save(datas)) {
                return new Result(200, "保存成功");
            } else {
                return new Result(500, "保存失败");
            }
...
//        try {
        if (ProcessConfigService.removeById(id)) {
...
//        try {
        Page<ProcessConfig> pages = new Page<ProcessConfig>(page, limit);
        QueryWrapper<ProcessConfig> ew=new QueryWrapper<ProcessConfig>();
        ...
        IPage<ProcessConfig> list=ProcessConfigService.listByJoinPage(pages,ew);
