...
/**
 * <p>
 * QA-BOM- 收集产品生产过程需参考的文件-子流程 前端控制器
 * </p>
 *
 * @author zhao
 * @since 2022-12-02
 */
@Controller
@RequestMapping("/dfQoaBomSon")
@ResponseBody
@CrossOrigin
public class DfQoaBomSonController {
    @Autowired
    com.ww.boengongye.service.DfQoaBomSonService DfQoaBomSonService;

    @Autowired
    private Environment env;

    private static final Logger logger = LoggerFactory.getLogger(DfQoaBomSonController.class);

    @RequestMapping(value = "/listAll")
    public Object listAll() {

        return new Result(0, "查询成功",DfQoaBomSonService.list());
    }



    @RequestMapping(value = "/saveOrUpdate", method = RequestMethod.POST)
    public Result save(@RequestBody DfQoaBomSon datas) {

        if (null != datas.getId()) {
            if (DfQoaBomSonService.updateById(datas)) {
...
//            datas.setCreateTime(TimeUtil.getNowTimeByNormal());
            if (DfQoaBomSonService.save(datas)) {
                return new Result(200, "保存成功");
            } else {
                return new Result(500, "保存失败");
            }
...
//        try {
        if (DfQoaBomSonService.removeById(id)) {
...
//        try {
        Page<DfQoaBomSon> pages = new Page<DfQoaBomSon>(page, limit);
        QueryWrapper<DfQoaBomSon> ew=new QueryWrapper<DfQoaBomSon>();
        ...
        IPage<DfQoaBomSon> list=DfQoaBomSonService.page(pages,ew);
...
//	        img.setArticleId(id);
            ...
            try {
                ImportExcelResult ter = DfQoaBomSonService.importOrder(env.getProperty("uploadPath") + "excel/" + fileName, file);
